...
LOGPATH = os.path.expanduser(
    settings.user.getsection("screen").get(
        "screenlog_path",
        os.path.join(
            settings.user.getsection("core").get("alphagsm_path", "~/.alphagsm"), "logs"
        ),
    )
)
SCREENRC = (
...
def rotatelogs(dirn, name):
    ...
    logs = [
        (int(el[length:]), el)
        for el in os.listdir(dirn)
        if el[:length] == match and el[length:].isdigit()
    ]
    logs.sort()
...
def start_screen(name, command, cwd=None):
    ...
    try:
        out = sp.check_output(
            ["screen", "-dmLS", SESSIONTAG + name, "-c", SCREENRC] + list(command),
            stderr=sp.STDOUT,
            shell=False,
            **extraargs
        )
        ...
        raise ScreenError(
            "Can't change to directory '" + cwd + "' while starting screen", ex
        )
    except OSError as ex:
...
def connect_to_screen(name):
    try:
        ...
        raise ScreenError(
            "Screen Failed with return value: " + str(ex.returncode), ex.returncode
        )
    except OSError as ex:
        raise ScreenError("Error executing screen: " + str(ex))
...
def write_screenrc(force=False):
    ...
    if not os.path.isfile(file_name) or (force == True):
        screenrc_text = open(
            os.path.join(os.path.abspath(os.path.dirname(__file__)), SCREENRC_TEMPLATE),
            "r",
        ).read()
        # adds the game server logpath directory to the screenrc_text
        screenrc_text = screenrc_text % os.path.join(
            LOGPATH, ""
        )  # appends trailing slash if none exists.
        f = open(file_name, "w")
