...
After `create ahl2server`, inspect or change these with `set`:
...
| --- | --- | --- | --- |
| `map` | gamemap, startmap, level, worldname | string | The currently selected map or level. Example: `act_airport`. |
...
| `servername` | hostname, name | string | The server's public name shown to players. Example: `AlphaGSM Action: Source`. |
