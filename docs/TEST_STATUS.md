...
| saleblazersserver | Wine validation 2026-05-28: the module now uses the best-known Linux launcher shape (`xvfb-run` + SDL `x11` + dummy audio + `LIBGL_ALWAYS_SOFTWARE=1`, without explicit `-headless`) and a bounded direct repro now reaches `Launching server...`, `Starting server console window process...`, `Waiting for Hosting Selections...`, and `Connected to Console Window!`; however, no listener appeared on the stored game port (`55000`) or query port (`27016`), so AlphaGSM still cannot prove query/info readiness for app `3099600`. |
