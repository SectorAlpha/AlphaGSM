...
| arksurvivalascended | Docker runtime (Wine/Proton) — PASSED 2026-05-30; anonymous SteamCMD app `2430930` and generic `tcp` health on the managed main port remain the supported Linux contract. The current branch routes the Docker launch through the shared Proton builder instead of executing the Windows `.exe` directly in a custom shell; replacement CI validation is pending. |
...
| citadelserver | Docker runtime (SteamCMD Linux) — PASSED 2026-05-31; anonymous SteamCMD app `489650`, the native Linux payload, and generic `tcp` health on the managed main port remain the supported contract. The 2026-07-16 forced process lane stayed active without opening that surface, so CI now keeps one Docker-default lifecycle. |
| colserver | SteamCMD |
...
| ckserver | SteamCMD |
| enshrouded | Docker runtime (Wine/Proton) — PASSED; AlphaGSM now treats `enshrouded_server.json` as authoritative, syncs `name` and `queryPort`, preserves unrelated generated settings, and defaults `queryPort` to the managed port `15637` rather than the stale `port + 1` value. Replacement CI validation is pending. |
...
| reignofdwarfserver | Docker runtime (Wine/Proton) — PASSED; the forced host-Proton process exited during the 2026-07-16 full run, so CI now keeps one Docker-default lifecycle and waits for AlphaGSM's A2S `info --json` surface rather than a host `screen` log. |
| sunkenlandserver | SteamCMD (Proton) |
| theforestserver | SteamCMD (Proton) |
| askaserver | SteamCMD (Wine) |
| blackops3server | Docker runtime (Wine/Proton) — PASSED; the current branch follows the shipped unranked server launch shape without the ineffective custom `-port` switch, maps the managed three-port group to fixed container ports `27015..27017`, and combines `CreateDedicatedModsLobby: ready!` with AlphaGSM generic UDP query/info. The stale disabled smoke placeholder is removed and CI now runs one Docker-default heavy lifecycle; replacement validation is pending. |
...
| returntomoriaserver | Docker runtime (Wine/Proton) — PASSED 2026-05-29; AlphaGSM manages `MoriaServerConfig.ini` and generic `udp` health on the managed game port. CI now keeps one Docker-default lifecycle and uses AlphaGSM `info --json` for readiness rather than a raw localhost UDP probe or status-file dependency. |
