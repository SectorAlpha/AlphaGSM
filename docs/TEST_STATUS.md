...
| chivalryserver | SteamCMD app 220070 installs the Linux dedicated server and the Port/QueryPort launch wiring now reaches the real startup path, but anonymous startup still exits before A2S is ready because `PhysXUpdateLoader.so` is missing from the Linux install |
