...
| terratechworldsserver | Wine/Proton validation 2026-05-28: launcher now uses the official Steam launch shape `-log -nullrhi`, plus headless `xvfb-run` / SDL / software-GL wiring, and the next bounded fix syncs AlphaGSM's configured `port` into `dedicated_server_config.json` after existing artifacts showed the dedicated server still defaulting to `7777`; keep it disabled until a fresh live rerun proves `Saved/Logs/TT2.log` and A2S readiness for app `2533070` |
