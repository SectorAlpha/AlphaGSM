...
| exfilserver | SteamCMD — PASSED; the server runs as the invoking host user in Docker and `query`, `info`, plus `info --json` use the validated TCP health surface on the managed game port instead of assuming A2S on `queryport` |
