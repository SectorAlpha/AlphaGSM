...
| citadelserver | Docker runtime (SteamCMD Linux) — PASSED 2026-05-31; anonymous SteamCMD app `489650`, the native Linux payload, and A2S health on the managed `queryport` remain the supported contract. The 2026-07-16 forced process lane stayed active without opening the old generic-TCP surface, so CI now keeps one Docker-default lifecycle; replacement validation of the corrected A2S contract is pending. |
| colserver | SteamCMD |
...
| returntomoriaserver | Docker runtime (Wine/Proton) — PASSED 2026-05-29; AlphaGSM manages `MoriaServerConfig.ini`, keeps process and Docker builders on one Proton preference, retains the interactive console setting required by world startup, waits for the game-owned `Status.json` running state without retaining invite/join secrets, then validates exact generic `udp` readiness on the managed game port through `info --json`. Redacted post-start diagnostics and failure-preserving cleanup are in-tree, with replacement validation pending. |
