...
| colserver | Docker runtime (SteamCMD Linux) — PASSED; CI keeps the validated Docker lifecycle after the forced host-process recheck stayed alive without opening Colony Survival's managed UDP query surface. |
...
| exfilserver | SteamCMD — PASSED; the server runs as the invoking host user in Docker, waits for the current server's exact `IpNetDriver` listener, and validates `query`, `info`, plus `info --json` through Exfil's separate Steam A2S query port rather than sending arbitrary packets to its Steam-networked game socket |
| fearthenightserver | Docker runtime (Wine/Proton) — PASSED 2026-05-29; AlphaGSM syncs `Moonlight/Saved/Config/WindowsServer/Engine.ini` and `GameUserSettings.ini`, launches the dedicated server with the real `Pittsburgh_Overworld?listen?Port=...?QueryPort=...?SessionName=...?MaxPlayers=...` map URL, and treats the live health surface as generic `udp` on the managed game port. CI keeps the validated Docker lifecycle after the forced host-process recheck failed to expose that surface. |
...
| outpostzeroserver | Docker runtime (Wine/Proton) — PASSED 2026-05-29; AlphaGSM mirrors the shipped `RunServer.bat` contract by launching `WindowsServer/SurvivalGameServer.exe RedPlanet ... -log`, syncing `Saved/Config/WindowsServer/Game.ini`, seeding `steam_appid.txt` beside the Win64 binaries, claiming both documented game-client UDP ports, and waiting for the real post-load discovery surface before `query`, `info`, and `info --json` on generic `udp` at `port + 1`. CI keeps the validated Docker lifecycle after the forced host-process recheck timed out. |
...
| pixarkserver | Docker runtime (Wine/Proton) — PASSED; CI keeps the validated Docker lifecycle after the forced host-process recheck remained in Proton initialization without reaching A2S. |
