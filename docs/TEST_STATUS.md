...
| blackwakeserver | Wine: focused validation now gets through setup/start with the managed `Server.cfg`, explicit `gamemode=7`, and module-owned `queryport`; the server no longer falls into the implicit Siege startup path, but the Unity dedicated process still logs Steam game-server init failures and repeated `BotHandler` `NullReferenceException` crashes before query-ready state — app 423410 |
| darkandlightserver | Wine/Proton validation 2026-05-28: fresh app `630230` setup/start reached `info --json` protocol `udp` on the managed game port and `query` passed as a generic UDP ping, but direct probes still show `a2s_info(127.0.0.1:27016)` timing out while `udp_ping(127.0.0.1:27016)` returns `Connection refused`, and `alphagsm stop` can drop the screen session while leaving `DNLServer.exe` listening on `0.0.0.0:<gameport>`; do not mark enabled yet |
| ducksideserver | SteamCMD app 2690320 requires authentication (No subscription) |
| empyrionserver | Wine/Proton validation 2026-05-28: the direct Proton plus `xvfb-run` path is still blocked before proven runtime/query readiness. In focused host runs, AlphaGSM stored explicit ports like `46319` and `42657` but the installed `dedicated.yaml` still stayed at `Srv_Port: 30000`, so the module is not yet syncing its real game-server port into Empyrion's config. The server-owned dedicated log under `server/Logs/5046/Dedicated_*.log` stopped at `Failed to create batch mode window: Success.`, and a manual 45 second reproduction of `DedicatedServer/EmpyrionDedicated.exe -batchmode -nographics -dedicated dedicated.yaml` under Proton plus `xvfb-run` also produced no fresh runtime log or bound-port evidence. Upstream still documents `30004` as `Tel_Port`, not a proven A2S endpoint — app 530870 |
...
| primalcarnageextinctionserver | Wine validation 2026-05-28: the checked-in launcher now matches the best-known dedicated command (`SERVER PC-Docks?...?bIsDedicated=true -seekfreeloadingserver -log`) and wraps it with `xvfb-run`; focused validation now also retries AlphaGSM's recommended claimed-port overrides so an unmanaged listener on the default `queryport` `27015` no longer blocks setup, but the remaining blocker is still post-launch because the fresh rerun with `queryport=27016` again fell back to a dead screen session and zero-byte `Launch.log` after Wine reported `Failed to read file 'PCDataStore_GameResource'` and an X `BadWindow` teardown for app `336400` |
...
| saleblazersserver | Wine validation 2026-05-28: fresh host smoke under `/media/cosmosquark/a55b079e-515f-4798-a120-b1e69dda0b22/useme` completed the anonymous SteamCMD install for app `3099600`, but AlphaGSM `start` still never reached A2S readiness on configured `port=35093` / `queryport=27016`; `info --json` returned no payload for 180 seconds and direct probes failed with `Info query failed: A2S query failed: timed out`, while `ss -lpun` showed no live listener on either port. The shipped `-headless -batchmode -nographics -logFile ./server.log` path still stops after `OperationException : Dependency \`Localization Settings Initialization\` is no longer valid after calling WaitForCompletion.`, `graphics device is Null`, and `Waiting for Hosting Selections...`. A manual `xvfb-run` + SDL `x11` + dummy audio + software-GL launch without `-headless` improved the high-water mark to `Starting server console window process...` and `Connected to Console Window!`, but still bound neither port nor produced a queryable A2S surface. Next bounded fix: test promoting that non-headless Linux launcher shape through the module/smoke path and only re-enable after a fresh AlphaGSM lifecycle proves bound ports plus `query` / `info` / `info --json`. |
