...
- Config files are either in a conventional location or stored explicitly in `server.data`.
...
4. Any generated config file path matches the path described in `docs/servers/` and `docs/server-templates/`.
...
- Docs describe `server.cfg` in one place while the module writes or reads a different file.
