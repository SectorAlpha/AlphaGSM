...
def main(name,args):
  ...
  for s in servers:
    if s.lower() in banned:
      print(s,"is a banned server name as it's too similar to a command",file=stderr)
      print(file=stderr)
      help(name,None)
      return 2
  if len(args)<1:
...
def runlistmulti(name,servers,args):
  ...
  for user,server in servers:
    ...
    output=[el for el in (el.strip() for el in output.decode(stdout.encoding).split("\n")) if len(el)>0]
    for el in output:
      if user is not None:
        print(user+"/"+el)
      else:
...
def help(name,server,cmd=None,*,file=stderr):
  if cmd is None:
    print("The Sector-Alpha Game Server Management Script (AlphaGSM)",file=file)
    print(file=file)
...
Developed by Cosmosquark and Staircase27. See the CREDITS file for a full list of contributors.

A command line tool to download, manage and maintain game servers using simple and similar commands.
See the README, future_plans and the changelog files for more details.
Hosted and maintained on our github page https://github.com/SectorAlpha/AlphaGSM.
Raise any issues or ask any questions on our github page, or contact cosmosquark@sector-alpha.net.
Additionally check out the project wiki http://wiki.sector-alpha.net/index.php?title=AlphaGSM""",file=file)
