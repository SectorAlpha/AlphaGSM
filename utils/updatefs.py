...
def update(old, new, target, linkdir=(), copy=(), skip=()):
    ...
    if doupdate(old, new, target, ".", linkdir, copy, skip, False):
        print(
            "WARNING: Some files couldn't be updated. Once fixed please remove '{0}', '{1}' and '{2}' files and folders".format(
                LOCALSUFFIX, OLDSUFFIX, NEWSUFFIX
            )
        )
        return False
    else:
...
def checkandcleartrees(rel, target, old, skip):
    ...
    for entry in os.listdir(target):
        if (
            entry[-len(OLDSUFFIX) :] == OLDSUFFIX
            or entry[-len(NEWSUFFIX) :] == NEWSUFFIX
            or entry[-len(LOCALSUFFIX) :] == LOCALSUFFIX
        ):
        ...
        if israwdir(targetentry):
            if exists(oldentry) and not matchespatterns(relentry + "/", skip):
                ...
                if not isdir(old):
                    ...
                    print(
                        "WARNING: file from old version is folder in local but isn't wanted in new.\nRenamed to '{0}".format(
            ...
            if checkfiles(targetentry, oldentry):
                ...
                anyleft = True
                print(
                    "WARNING: file from old version has local changes but isn't wanted in new.\nRenamed to '{0}".format(
                        targetentry + LOCALSUFFIX
                    )
                )
        ...
        if israwdir(targetentry):
            if isdir(old):
                ...
                if checkandcleartrees(relentry, targetentry, oldentry, skip):
                    ...
                    print(
                        "WARNING: directory {0} from old isn't wanted in new but some files weren't removed".format(
                            targetentry
                        )
                    )
                    return True
            ...
            os.rename(targetentry, targetentry + LOCALSUFFIX)
            print(
                "WARNING: File from old version has been changed in local but isn't wanted in new.\nRenamed to '{0}".format(
                    targetentry + LOCALSUFFIX
                )
            )
            return True
...
def updatefromnew(old, new, target, rel, entry, linkdir, copy, skip, forcecopy):
    ...
    if isdir(newentry) and (forcecopy or not matchespatterns(matchentry, linkdir)):
        ...
        if forcecopy or matchespatterns(matchentry, copy):
            return doupdate(
                oldentry, newentry, targetentry, relentry, linkdir, copy, skip, True
            )
        else:
            return doupdate(
                oldentry, newentry, targetentry, relentry, linkdir, copy, skip, False
            )
            ...
            if exists(oldentry) and checkfiles(targetentry, oldentry):
                ...
                print(
                    "WARNING: File present in new version and local but not in old.\nLeaving your version ({0}) but putting new file in directory with it ({1}).\n".format(
                        targetentry, targetentry + NEWSUFFIX
                    )
                )
                return True
                ...
                print(
                    "WARNING: File has update but local changes present.\nLeaving your version ({0}) but putting new and old files in directory with it\n({1} and {2})".format(
                        targetentry, targetentry + NEWSUFFIX, targetentry + OLDSUFFIX
                    )
                )
                return True
