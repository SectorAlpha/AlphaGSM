...
class CmdSpec(tuple):
    ...
    def __new__(
        _cls, requiredarguments=(), optionalarguments=(), repeatable=False, options=()
    ):
        "Create a command specification"
        return tuple.__new__(
            _cls, (requiredarguments, optionalarguments, repeatable, options)
        )

    def __repr__(self):
        "Return a nicely formatted representation string"
        return (
            "CmdSpec(requiredarguments=%r, optionalarguments=%r, repeatable=%r, options=%r)"
            % self
        )
    ...
    def combine(self, other):
        ...
        if other is not None:
            if len(other.requiredarguments) > 0:
                raise OptionError(
                    "Error combining arguments: Can't add more required arguments"
                )
            if self.repeatable:
                ...
                return CmdSpec(
                    ...
                    self.optionalarguments + other.optionalarguments,
                    other.repeatable,
                    self.options + other.options,
                )
        return self

    requiredarguments = property(
        _itemgetter(0), doc="The required arguments for the command"
    )

    optionalarguments = property(
        _itemgetter(1), doc="The optional arguments for the command"
    )
...
class ArgSpec(tuple):
    ...
    conversion = property(
        _itemgetter(2), doc="A function to convert the value to the appropriate type"
    )
...
class OptSpec(tuple):
    ...
    def __new__(
        _cls, shortforms, longforms, description, keyword, argument, value_or_conversion
    ):
    ...
    def __repr__(self):
        "Return a nicely formatted representation string"
        return (
            "OptSpec(shortforms=%r, longforms=%r, description=%r, keyword=%r, argument=%r, value_or_conversion=%r)"
            % self
        )
    ...
    argument = property(
        _itemgetter(4), doc="The name of the argument to the option or None"
    )

    value_or_conversion = property(
        _itemgetter(5),
        doc="If argument is None then the value to store if this option is given"
        " else the function to convert the options argument to the appropriate type",
    )

    value = property(
        _itemgetter(5),
        doc="The value to store if this option is given. Only valid if argument is None. Alias for value_or_conversion",
    )

    conversion = property(
        _itemgetter(5),
        doc="The function to convert the options argument to the appropriate type. Only valid if argument is None. Alias for value_or_conversion",
    )
