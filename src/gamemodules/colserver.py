...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ...
            ArgSpec("DIR", "The directory to install Colony Survival in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Colony Survival dedicated server to the latest version.",
    "restart": "Restart the Colony Survival dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="colonyserver.x86_64"):
    ...
    server.data.setdefault("backupfiles", ["savegames", "server_settings.json"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["savegames", "server_settings.json"]}},
            "schedule": [("default", 0, "days")],
        }

    if port is None:
        port = server.data.get("port", 27004)
    if ask:
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Colony Survival server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Colony Survival server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Colony Survival server files and optionally restart the server."""
...
def restart(server):
    """Restart the Colony Survival server."""
