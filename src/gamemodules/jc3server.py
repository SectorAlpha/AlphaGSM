...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ...
            ArgSpec("DIR", "The directory to install Just Cause 3 in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Just Cause 3 dedicated server to the latest version.",
    "restart": "Restart the Just Cause 3 dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="openjc3-server"):
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Just Cause 3 server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Just Cause 3 server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Just Cause 3 server files and optionally restart the server."""
...
def restart(server):
    """Restart the Just Cause 3 server."""
