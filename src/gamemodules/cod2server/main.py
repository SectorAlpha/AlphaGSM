...
COD2_MOD_CACHE_DIRNAME = "cod2server"
COD2_ALLOWED_MOD_SUFFIXES = {
    ".7z": "7z",
    ".pk3": "pk3",
    ".tar.gz": "tar",
    ".tbz2": "tar",
...
command_args = gamemodule_common.build_setup_download_command_args(
    ...
    "The directory to install Call of Duty 2 in",
)
command_args.update(
    {
        "mod": CmdSpec(
...
command_descriptions = {
    "mod": "Manage Call of Duty 2 pk3 content from direct archive URLs or direct pk3 URLs into the active moddir directory."
}
command_functions = {}
max_stop_wait = 1
...
_quake_launch_schema = gamemodule_common.build_quake_setting_schema(
    ...
    game_key="moddir",
    game_description="The active Call of Duty 2 mod directory.",
...
def ensure_mod_state(server):
    """Seed the Call of Duty 2 mod desired-state shape and return it."""
...
def _ensure_moddir_backup(server):
    ...
    backupfiles = list(server.data.setdefault("backupfiles", ["main"]))
    ...
    default_profile = backup.setdefault("profiles", {}).setdefault("default", {"targets": ["main"]})
    targets = list(default_profile.setdefault("targets", ["main"]))
...
def _cache_root(server) -> Path:
    return Path(server.data["dir"]) / ".alphagsm" / "mods" / COD2_MOD_CACHE_DIRNAME
...
def _desired_record(server, resolved_id: str) -> dict:
    for entry in ensure_mod_state(server)["desired"]["url"]:
        if entry.get("resolved_id") == resolved_id:
            return entry
    raise ModSupportError(f"Missing Call of Duty 2 desired-state metadata for '{resolved_id}'")
    ...
    if archive_type in ("zip", "7z", "tar"):
        ...
        if install_root is None:
            raise ModSupportError(
                f"No Call of Duty 2 pk3 content was found in the downloaded payload; expected {moddir}/<name>.pk3 or bare .pk3 files"
            )
    else:
...
def cod2_mod_command(server, action, source=None, identifier=None, extra=None, **kwargs):
    """Handle the Call of Duty 2 `mod` command desired-state subcommands."""
    ...
    if action == "add" and source == "url":
        if not identifier:
            raise ServerError("Call of Duty 2 mod add url requires a pk3 archive URL or direct pk3 URL")
        resolved = resolve_direct_url_entry(
            identifier,
            filename=extra,
            allowed_suffixes=COD2_ALLOWED_MOD_SUFFIXES,
            entry_label="Call of Duty 2 mod URL entries",
        ...
        for entry in mods["desired"]["url"]:
            if entry.get("requested_id") == resolved["requested_id"]:
                raise ServerError(
                    f"Call of Duty 2 content '{resolved['requested_id']}' is already present in desired state"
                )
    ...
    if action == "remove" and source == "url":
        if not identifier:
            raise ServerError("Call of Duty 2 mod remove url requires the original content URL")
        existing = mods["desired"]["url"]
        updated = [entry for entry in existing if entry.get("requested_id") != identifier]
        if len(updated) == len(existing):
            raise ServerError(f"Call of Duty 2 content '{identifier}' is not present in desired state")
        mods["desired"]["url"] = updated
        _save_data(server)
        return
    raise ServerError("Unsupported Call of Duty 2 mod command")


command_functions["mod"] = cod2_mod_command


def configure(server, ask, port=None, dir=None, *, url=None, download_name=None, exe_name="cod2_lnxded"):
    """Collect and store configuration values for a Call of Duty 2 server."""
    ...
    gamemodule_common.configure_install_dir(
        ...
        ask,
        dir,
        prompt="Where would you like to install the Call of Duty 2 server:",
    )
    default_url = (
        "https://archive.org/download/call-of-duty-2-files/"
        "Files/Linux%20Dedicated%20Server%20%281.3%29/"
    ...
    gamemodule_common.configure_download_source(
        ...
        default_name="cod2-lnxded-1.3-06232006.tar.bz2",
        prompt="Direct archive URL for the Call of Duty 2 server:",
    )
...
def install(server):
    """Download and install the Call of Duty 2 server archive."""
...
def sync_server_config(server):
    """Rewrite the managed Call of Duty 2 server.cfg values from datastore state."""
...
def get_start_command(server):
    """Build the command used to launch a Call of Duty 2 dedicated server."""
...
def message(server, msg):
    """Call of Duty 2 has no simple generic message console support here."""

    gamemodule_common.print_unsupported_message()


def backup(server, profile=None):
    """Run the shared backup implementation for a Call of Duty 2 server."""

    gamemodule_common.run_backup(server, profile, backup_module=backup_utils)


def checkvalue(server, key, *value):
    """Validate supported Call of Duty 2 datastore edits."""
