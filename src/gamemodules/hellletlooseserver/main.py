...
command_args = gamemodule_common.build_setup_update_restart_command_args(
    "The game port to use for the Hell Let Loose server",
    "The directory to install Hell Let Loose in",
)
command_descriptions = gamemodule_common.build_update_restart_command_descriptions(
    "Update the Hell Let Loose dedicated server to the latest version.",
    "Restart the Hell Let Loose dedicated server.",
)
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="HLLServer.exe"):
    ...
    gamemodule_common.ensure_backup_config(
        server,
        backupfiles=["HLL", "Server", "Saved"],
        targets=["HLL", "Server", "Saved"],
    )
    ...
    gamemodule_common.configure_install_dir(
        ...
        ask,
        dir,
        prompt="Where would you like to install the Hell Let Loose server:",
    )
...
install.__doc__ = "Download the Hell Let Loose server files via SteamCMD."
...
update.__doc__ = "Update the Hell Let Loose server files and optionally restart the server."


restart = gamemodule_common.make_restart_hook()
restart.__doc__ = "Restart the Hell Let Loose server."
...
def checkvalue(server, key, *value):
    ...
    return gamemodule_common.handle_basic_checkvalue(
        ...
        *value,
        int_keys=("port",),
        str_keys=("startmap", "exe_name", "dir", "maxplayers"),
