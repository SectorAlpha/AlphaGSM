...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the ATLAS server", int),
            ArgSpec("DIR", "The directory to install ATLAS in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the ATLAS dedicated server to the latest version.",
    "restart": "Restart the ATLAS dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(
    ...
    *,
    ...
    server.data.setdefault("map", "Ocean")
    ...
    server.data.setdefault("queryport", "57561")
    ...
    if port is None:
        port = server.data.get("port", 57555)
    if ask:
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the ATLAS server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the ATLAS server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the ATLAS server files and optionally restart the server."""
...
def checkvalue(server, key, *value):
    ...
    if key[0] in ("port", "queryport", "maxplayers"):
        return int(value[0])
    if key[0] in ("map", "sessionname", "serverpassword", "adminpassword", "exe_name", "dir"):
        return str(value[0])
