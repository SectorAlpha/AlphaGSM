...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the FiveM server", int),
            ArgSpec("DIR", "The directory to install FiveM in", str),
        ),
        options=(
...
def resolve_download(version=None):
    ...
    if version in (None, "", "latest"):
        with urllib.request.urlopen(CFX_ARTIFACTS_BASE + "/latest.json") as response:
...
def configure(
    ...
    *,
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the FiveM server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
    ...
    if ask and url is None:
        inp = input("Direct archive URL for the FiveM server: [%s] " % (server.data["url"],)).strip()
        if inp:
            server.data["url"] = inp
