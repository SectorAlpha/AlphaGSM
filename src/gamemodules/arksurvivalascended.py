...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the ARK: Survival Ascended server", int),
            ArgSpec("DIR", "The directory to install ARK: Survival Ascended in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the ARK: Survival Ascended dedicated server to the latest version.",
    "restart": "Restart the ARK: Survival Ascended dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(
    ...
    *,
    ...
    server.data.setdefault("map", "TheIsland_WP")
    ...
    server.data.setdefault("backupfiles", ["ShooterGame/Saved", "ShooterGame/Saved/Config/WindowsServer"])
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the ARK: Survival Ascended server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the ARK: Survival Ascended server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the ARK: Survival Ascended server files and optionally restart the server."""
...
def restart(server):
    """Restart the ARK: Survival Ascended server."""
...
def checkvalue(server, key, *value):
    ...
    if key[0] in ("port", "queryport", "maxplayers"):
        return int(value[0])
    if key[0] in ("map", "sessionname", "adminpassword", "serverpassword", "exe_name", "dir"):
        return str(value[0])
