...
command_descriptions = {
    "update": "Update The Front dedicated server to the latest version.",
    "restart": "Restart The Front dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def install(server):
    """Download The Front server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update The Front server files and optionally restart the server."""
...
def checkvalue(server, key, *value):
    ...
    if key[0] in ("port", "queryport", "maxplayers"):
        return int(value[0])
    if key[0] in ("servername", "worldname", "exe_name", "dir"):
        return str(value[0])
