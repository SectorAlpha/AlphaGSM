...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ...
            ArgSpec("DIR", "The directory to install Space Engineers in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Space Engineers dedicated server to the latest version.",
    "restart": "Restart the Space Engineers dedicated server.",
}
command_functions = {}
max_stop_wait = 1


def configure(server, ask, port=None, dir=None, *, exe_name="DedicatedServer64"):
    ...
    server.data.setdefault("backupfiles", ["Saves", "Storage", "SpaceEngineers-Dedicated.cfg"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["Saves", "Storage", "SpaceEngineers-Dedicated.cfg"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Space Engineers server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Space Engineers server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Space Engineers server files and optionally restart the server."""
...
def restart(server):
    """Restart the Space Engineers server."""
