...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Black Ops III server", int),
            ArgSpec("DIR", "The directory to install Black Ops III in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Black Ops III dedicated server to the latest version.",
    "restart": "Restart the Black Ops III dedicated server.",
}
command_functions = {}
max_stop_wait = 1


def configure(server, ask, port=None, dir=None, *, exe_name="BlackOps3Server.exe"):
    ...
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["players", "zone", "mods"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Black Ops III server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
