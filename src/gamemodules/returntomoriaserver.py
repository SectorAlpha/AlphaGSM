...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Return to Moria server", int),
            ArgSpec("DIR", "The directory to install Return to Moria in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Return to Moria dedicated server to the latest version.",
    "restart": "Restart the Return to Moria dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="MoriaServer.exe"):
    ...
    server.data.setdefault("advertiseport", "7777")
    server.data.setdefault("backupfiles", ["Moria/Saved/Worlds", "MoriaServerConfig.ini", "permissions.txt"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["Moria/Saved/Worlds", "MoriaServerConfig.ini", "permissions.txt"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Return to Moria server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Return to Moria server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Return to Moria server files and optionally restart the server."""
...
def restart(server):
    """Restart the Return to Moria server."""
...
def status(server, verbose):
    """Detailed Return to Moria status is not implemented yet."""
...
def checkvalue(server, key, *value):
    ...
    if key[0] in ("port", "advertiseport"):
        return int(value[0])
