...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ...
            ArgSpec("DIR", "The directory to install V Rising in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the V Rising dedicated server to the latest version.",
    "restart": "Restart the V Rising dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="VRisingServer"):
    ...
    server.data.setdefault("backupfiles", ["save-data", "Settings"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["save-data", "Settings"]}},
            "schedule": [("default", 0, "days")],
        }

    if port is None:
        port = server.data.get("port", 9876)
    if ask:
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the V Rising server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the V Rising server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the V Rising server files and optionally restart the server."""
