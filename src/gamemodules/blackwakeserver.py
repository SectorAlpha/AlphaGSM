...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Blackwake server", int),
            ArgSpec("DIR", "The directory to install Blackwake in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Blackwake dedicated server to the latest version.",
    "restart": "Restart the Blackwake dedicated server.",
}
command_functions = {}
max_stop_wait = 1


def configure(server, ask, port=None, dir=None, *, exe_name="Blackwake Dedicated Server.exe"):
    """Collect and store configuration values for a Blackwake server."""

    server.data["Steam_AppID"] = steam_app_id
    server.data["Steam_anonymous_login_possible"] = steam_anonymous_login_possible
    server.data.setdefault("queryport", "27015")
    server.data.setdefault("maxplayers", "54")
    server.data.setdefault("backupfiles", ["Blackwake_Data", "save", "config"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["save", "config"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Blackwake server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def update(server, validate=False, restart=False):
    """Update the Blackwake server files and optionally restart the server."""
