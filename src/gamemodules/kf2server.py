...
command_descriptions = {
    "update": "Update the Killing Floor 2 dedicated server to the latest version.",
    "restart": "Restart the Killing Floor 2 dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def get_start_command(server):
    ...
    return (
        [
            ...
            "-QueryPort=%s" % (server.data.get("queryport", "27015"),),
        ],
        server.data["dir"],
    )
...
def checkvalue(server, key, *value):
    ...
    if key[0] in ("port", "queryport"):
        return int(value[0])
    if key[0] in ("startmap", "gametype", "configsubdir", "exe_name", "dir"):
        return str(value[0])
