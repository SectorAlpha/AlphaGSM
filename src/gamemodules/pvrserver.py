...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ...
            ArgSpec("DIR", "The directory to install Pavlov VR in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Pavlov VR dedicated server to the latest version.",
    "restart": "Restart the Pavlov VR dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="PavlovServer.sh"):
    ...
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["Pavlov", "Saved"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Pavlov VR server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
