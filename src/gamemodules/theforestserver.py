...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for The Forest server", int),
            ArgSpec("DIR", "The directory to install The Forest in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update The Forest dedicated server to the latest version.",
    "restart": "Restart The Forest dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="TheForestDedicatedServer.exe"):
    ...
    server.data.setdefault("backupfiles", ["Server.cfg", "ds"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["Server.cfg", "ds"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install The Forest server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download The Forest server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update The Forest server files and optionally restart the server."""
...
def get_start_command(server):
    ...
    return (
        ["./" + server.data["exe_name"], "-batchmode", "-nosteamclient", "-nographics"],
        server.data["dir"],
    )
