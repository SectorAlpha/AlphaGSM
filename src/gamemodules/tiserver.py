...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ...
            ArgSpec("DIR", "The directory to install The Isle in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update The Isle dedicated server to the latest version.",
    "restart": "Restart The Isle dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="TheIsleServer.sh"):
    ...
    server.data.setdefault("map", "TheIsle")
    server.data.setdefault("backupfiles", ["TheIsle/Saved/Config", "TheIsle/Saved/Logs"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["TheIsle/Saved/Config", "TheIsle/Saved/Logs"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install The Isle server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download The Isle server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update The Isle server files and optionally restart the server."""
