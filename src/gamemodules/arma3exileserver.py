...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ...
            ArgSpec("DIR", "The directory to install Arma 3 Exile in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Arma 3 Exile dedicated server to the latest version.",
    "restart": "Restart the Arma 3 Exile dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="arma3server_x64"):
    ...
    server.data.setdefault("mod", "@Exile")
    server.data.setdefault("servermod", "@ExileServer")
    server.data.setdefault("backupfiles", ["@ExileServer", "mpmissions", "profiles", "server.cfg"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["@ExileServer", "mpmissions", "profiles", "server.cfg"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Arma 3 Exile server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Arma 3 server files for an Exile install via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Arma 3 Exile server files and optionally restart the server."""
...
def restart(server):
    """Restart the Arma 3 Exile server."""
