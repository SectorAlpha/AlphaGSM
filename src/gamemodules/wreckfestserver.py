...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Wreckfest server", int),
            ArgSpec("DIR", "The directory to install Wreckfest in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Wreckfest dedicated server to the latest version.",
    "restart": "Restart the Wreckfest dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="WreckfestServer"):
    ...
    server.data.setdefault("configfile", "server_config.cfg")
    server.data.setdefault("backupfiles", ["server_config.cfg", "savegame"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["server_config.cfg", "savegame"]}},
            "schedule": [("default", 0, "days")],
        }

    if port is None:
        port = server.data.get("port", 33540)
    if ask:
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Wreckfest server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def _patch_wreckfest_port(server):
...
def install(server):
    ...
    _patch_wreckfest_port(server)


def update(server, validate=False, restart=False):
    """Update the Wreckfest server files and optionally restart the server."""
