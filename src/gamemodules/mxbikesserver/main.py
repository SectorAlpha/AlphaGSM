...
command_args = gamemodule_common.build_setup_download_command_args(
    "The game port to use for the MX Bikes server",
    "The directory to install MX Bikes in",
)
...
def configure(
    ...
    *,
    ...
    gamemodule_common.ensure_backup_config(
        server,
        backupfiles=["profiles", "mods", "server"],
        targets=["profiles", "mods", "server"],
    )
    ...
    gamemodule_common.configure_install_dir(
        ...
        ask,
        dir,
        prompt="Where would you like to install the MX Bikes server:",
    )
    if url is not None:
        server.data["url"] = url
    ...
    gamemodule_common.configure_download_source(
        ...
        default_name="mxbikes-dedicated.zip",
        prompt="",
    )
