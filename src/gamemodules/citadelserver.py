...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Citadel server", int),
...
def configure(server, ask, port=None, dir=None, *, exe_name="CitadelServer-Linux-Shipping"):
    ...
    server.data.setdefault("map", "rook")
    server.data.setdefault("backupfiles", ["Saved", "Saved/Config/LinuxServer"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["Saved", "Saved/Config/LinuxServer"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Citadel server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
