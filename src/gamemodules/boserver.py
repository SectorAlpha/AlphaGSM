...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ...
            ArgSpec("DIR", "The directory to install Ballistic Overkill in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Ballistic Overkill dedicated server to the latest version.",
    "restart": "Restart the Ballistic Overkill dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="BODS.x86_64"):
    ...
    server.data.setdefault("map", "oilrig")
    server.data.setdefault("backupfiles", ["BODS_Data"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["BODS_Data"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Ballistic Overkill server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Ballistic Overkill server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Ballistic Overkill server files and optionally restart the server."""
...
def restart(server):
    """Restart the Ballistic Overkill server."""
...
def checkvalue(server, key, *value):
    ...
    if key[0] in ("port", "maxplayers"):
        return int(value[0])
    if key[0] in ("hostname", "map", "exe_name", "dir"):
        return str(value[0])
