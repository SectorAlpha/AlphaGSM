...
JK2_MOD_CACHE_DIRNAME = "jk2server"
JK2_ALLOWED_MOD_SUFFIXES = {
    ".7z": "7z",
    ".pk3": "pk3",
    ".tar.gz": "tar",
    ".tbz2": "tar",
...
command_descriptions = {
    "mod": "Manage Jedi Outcast pk3 content from direct archive URLs or direct pk3 URLs into the active fs_game directory."
}
command_functions = {}
max_stop_wait = 1
setting_schema = {
...
def ensure_mod_state(server):
    """Seed the Jedi Outcast mod desired-state shape and return it."""
...
def _fs_game(server) -> str:
    return str(server.data.get("fs_game") or "base")


def _ensure_fs_game_backup(server):
    fs_game = _fs_game(server)
    backupfiles = list(server.data.setdefault("backupfiles", ["base"]))
    ...
    default_profile = backup.setdefault("profiles", {}).setdefault("default", {"targets": ["base"]})
    targets = list(default_profile.setdefault("targets", ["base"]))
...
def _cache_root(server) -> Path:
    return Path(server.data["dir"]) / ".alphagsm" / "mods" / JK2_MOD_CACHE_DIRNAME
...
def _desired_record(server, resolved_id: str) -> dict:
    for entry in ensure_mod_state(server)["desired"]["url"]:
        if entry.get("resolved_id") == resolved_id:
            return entry
    raise ModSupportError(f"Missing Jedi Outcast desired-state metadata for '{resolved_id}'")
    ...
    if archive_type in ("zip", "7z", "tar"):
        ...
        if install_root is None:
            raise ModSupportError(
                f"No Jedi Outcast pk3 content was found in the downloaded payload; expected {fs_game}/<name>.pk3 or bare .pk3 files"
            )
    else:
...
def jk2_mod_command(server, action, source=None, identifier=None, extra=None, **kwargs):
    """Handle the Jedi Outcast `mod` command desired-state subcommands."""
    ...
    if action == "add" and source == "url":
        if not identifier:
            raise ServerError("Jedi Outcast mod add url requires a pk3 archive URL or direct pk3 URL")
        resolved = resolve_direct_url_entry(
            identifier,
            filename=extra,
            allowed_suffixes=JK2_ALLOWED_MOD_SUFFIXES,
            entry_label="Jedi Outcast mod URL entries",
        ...
        for entry in mods["desired"]["url"]:
            if entry.get("requested_id") == resolved["requested_id"]:
                raise ServerError(
                    f"Jedi Outcast content '{resolved['requested_id']}' is already present in desired state"
                )
    ...
    if action == "remove" and source == "url":
        if not identifier:
            raise ServerError("Jedi Outcast mod remove url requires the original content URL")
        existing = mods["desired"]["url"]
        updated = [entry for entry in existing if entry.get("requested_id") != identifier]
        if len(updated) == len(existing):
            raise ServerError(f"Jedi Outcast content '{identifier}' is not present in desired state")
        mods["desired"]["url"] = updated
        _save_data(server)
        return
    raise ServerError("Unsupported Jedi Outcast mod command")


command_functions["mod"] = jk2_mod_command
