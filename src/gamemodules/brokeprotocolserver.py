...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the BROKE PROTOCOL server", int),
            ArgSpec("DIR", "The directory to install BROKE PROTOCOL in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the BROKE PROTOCOL dedicated server to the latest version.",
    "restart": "Restart the BROKE PROTOCOL dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="Start.sh"):
    ...
    server.data.setdefault("backupfiles", ["SaveData", "Config"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["SaveData", "Config"]}},
            "schedule": [("default", 0, "days")],
        }

    if port is None:
        port = server.data.get("port", 27777)
    if ask:
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the BROKE PROTOCOL server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the BROKE PROTOCOL server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the BROKE PROTOCOL server files and optionally restart the server."""
...
def restart(server):
    """Restart the BROKE PROTOCOL server."""
