...
command_descriptions = {
    "update": "Update the Unturned dedicated server to the latest version.",
    "restart": "Restart the Unturned dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="ServerHelper.sh"):
    ...
    server.data.setdefault("serverid", server.name)
    server.data.setdefault("launchmode", "InternetServer")
    server.data.setdefault("backupfiles", ["Servers", "ServerHelper.sh"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["Servers"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Unturned server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def update(server, validate=False, restart=False):
    """Update the Unturned server files and optionally restart the server."""
...
def checkvalue(server, key, *value):
    ...
    if key[0] in ("serverid", "launchmode", "exe_name", "dir"):
        return str(value[0])
