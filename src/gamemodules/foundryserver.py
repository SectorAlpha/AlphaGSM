...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the FOUNDRY server", int),
            ArgSpec("DIR", "The directory to install FOUNDRY in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the FOUNDRY dedicated server to the latest version.",
    "restart": "Restart the FOUNDRY dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="FoundryDedicatedServer"):
    ...
    server.data.setdefault("backupfiles", ["saves", "configs"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["saves", "configs"]}},
            "schedule": [("default", 0, "days")],
        }

    if port is None:
        port = server.data.get("port", 37200)
    if ask:
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the FOUNDRY server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the FOUNDRY server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the FOUNDRY server files and optionally restart the server."""
