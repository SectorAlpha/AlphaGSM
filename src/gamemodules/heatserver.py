...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Heat server", int),
            ArgSpec("DIR", "The directory to install Heat in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Heat dedicated server to the latest version.",
    "restart": "Restart the Heat dedicated server.",
}
command_functions = {}
max_stop_wait = 1


def configure(server, ask, port=None, dir=None, *, exe_name="HeatServer.exe"):
    ...
    server.data.setdefault("startmap", "America")
    server.data.setdefault("backupfiles", ["Saved", "ServerConfig.cfg"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["Saved", "ServerConfig.cfg"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Heat server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Heat server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Heat server files and optionally restart the server."""
