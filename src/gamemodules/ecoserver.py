...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ...
            ArgSpec("DIR", "The directory to install Eco in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Eco dedicated server to the latest version.",
    "restart": "Restart the Eco dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="EcoServer"):
    ...
    server.data.setdefault("storage", "Storage")
    server.data.setdefault("backupfiles", ["Storage", "Configs"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["Storage", "Configs"]}},
            "schedule": [("default", 0, "days")],
        }

    if port is None:
        port = server.data.get("port", 3000)
    if ask:
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Eco server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Eco server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Eco server files and optionally restart the server."""
