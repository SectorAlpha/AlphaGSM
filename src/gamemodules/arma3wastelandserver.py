...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ...
            ArgSpec("DIR", "The directory to install Arma 3 Wasteland in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Arma 3 Wasteland dedicated server to the latest version.",
    "restart": "Restart the Arma 3 Wasteland dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="arma3server_x64"):
    ...
    server.data.setdefault("world", "Altis")
    server.data.setdefault("mod", "")
    server.data.setdefault("mission", "A3Wasteland_v1.3.Stratis")
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Arma 3 Wasteland server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Arma 3 server files for a Wasteland install via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Arma 3 Wasteland server files and optionally restart the server."""
...
def restart(server):
    """Restart the Arma 3 Wasteland server."""
...
def checkvalue(server, key, *value):
    ...
    if key[0] == "port":
        return int(value[0])
    if key[0] in ("configfile", "profilesdir", "world", "mod", "mission", "exe_name", "dir"):
        return str(value[0])
