...
UT2K4_MOD_CACHE_DIRNAME = "ut2k4server"
UT2K4_ALLOWED_MOD_SUFFIXES = {
    ".7z": "7z",
    ".ka": "ka",
    ".ogg": "ogg",
    ".tar.gz": "tar",
...
UT2K4_DESTINATION_BY_LOWER = {value.lower(): value for value in UT2K4_ALLOWED_DESTINATIONS}
UT2K4_SINGLE_FILE_DESTINATIONS = {
    ".ka": "KarmaData",
    ".ogg": "Music",
    ".uax": "Sounds",
    ".ukx": "Animations",
...
UT2K4_IGNORED_TOP_LEVEL_SUFFIXES = {
    ".bmp",
    ".gif",
    ".htm",
    ".html",
...
command_descriptions = {
    "mod": "Manage Unreal Tournament 2004 custom content from direct archive URLs or direct content-file URLs."
}
command_functions = {}
max_stop_wait = 1
...
def ensure_mod_state(server):
    """Seed the UT2004 content desired-state shape and return it."""
...
def _cache_root(server) -> Path:
    return Path(server.data["dir"]) / ".alphagsm" / "mods" / UT2K4_MOD_CACHE_DIRNAME
...
def _content_destination_for_name(filename: str) -> str | None:
    ...
    for suffix, destination in UT2K4_SINGLE_FILE_DESTINATIONS.items():
...
def _should_ignore_archive_path(relative_path: Path) -> bool:
    ...
    if basename in ("thumbs.db", ".ds_store"):
        return True
    return Path(basename).suffix in UT2K4_IGNORED_TOP_LEVEL_SUFFIXES
...
def _build_install_stage(stage_root: Path) -> Path | None:
    ...
    for candidate in candidates:
        ...
        for source_path in sorted(path for path in candidate.rglob("*") if path.is_file()):
            ...
            if relative_path.parts and relative_path.parts[0].lower() in UT2K4_DESTINATION_BY_LOWER:
                destination_relative = Path(
                    UT2K4_DESTINATION_BY_LOWER[relative_path.parts[0].lower()]
                ...
                if destination is not None:
                    ...
                    raise ModSupportError(
                        f"Unsupported UT2004 archive payload: {relative_path.as_posix()}"
                    )
            elif _should_ignore_archive_path(relative_path):
                continue
            else:
                raise ModSupportError(
                    "UT2004 content archives must unpack into Animations/, KarmaData/, Maps/, Music/, Sounds/, StaticMeshes/, or Textures/"
                )
...
def _desired_record(server, resolved_id: str) -> dict:
    for entry in ensure_mod_state(server)["desired"]["url"]:
        if entry.get("resolved_id") == resolved_id:
            return entry
    raise ModSupportError(f"Missing UT2004 desired-state metadata for '{resolved_id}'")
...
def _install_url_entry(server, desired_entry: DesiredModEntry) -> InstalledModEntry:
    ...
    if archive_type in ("zip", "7z", "tar"):
        ...
        if install_root is None:
            raise ModSupportError(
                "No UT2004 content files were found in the downloaded payload; expected approved content directories or bare map/content files"
            )
    else:
        destination = _content_destination_for_name(desired_record["filename"])
        if destination is None:
            raise ModSupportError(
                f"Unsupported direct UT2004 content file: {desired_record['filename']}"
            )
    ...
    installed_paths = install_staged_tree(
        ...
        allowed_destinations=UT2K4_ALLOWED_DESTINATIONS,
    )
...
def _remove_owned_entry(server, installed_entry: InstalledModEntry) -> None:
    ...
    approved_roots = [server_root / destination for destination in UT2K4_ALLOWED_DESTINATIONS]
...
def ut2k4_mod_command(server, action, source=None, identifier=None, extra=None, **kwargs):
    """Handle the UT2004 `mod` command desired-state subcommands."""
    ...
    if action == "add" and source == "url":
        if not identifier:
            raise ServerError(
                "Unreal Tournament 2004 mod add url requires a content archive URL or direct content file URL"
            )
        resolved = resolve_direct_url_entry(
            identifier,
            filename=extra,
            allowed_suffixes=UT2K4_ALLOWED_MOD_SUFFIXES,
            entry_label="Unreal Tournament 2004 mod URL entries",
            filename_description="a supported UT2004 content filename or archive filename",
        )
        for entry in mods["desired"]["url"]:
            if entry.get("requested_id") == resolved["requested_id"]:
                raise ServerError(
                    f"Unreal Tournament 2004 content '{resolved['requested_id']}' is already present in desired state"
                )
    ...
    if action == "remove" and source == "url":
        if not identifier:
            raise ServerError("Unreal Tournament 2004 mod remove url requires the original content URL")
        existing = mods["desired"]["url"]
        updated = [entry for entry in existing if entry.get("requested_id") != identifier]
        if len(updated) == len(existing):
            raise ServerError(
                f"Unreal Tournament 2004 content '{identifier}' is not present in desired state"
            )
        mods["desired"]["url"] = updated
        _save_data(server)
        return
    raise ServerError("Unsupported Unreal Tournament 2004 mod command")


command_functions["mod"] = ut2k4_mod_command
...
def _runtime_requirement_extras(server):
    """Return conditional host dependency metadata for UT2004 setup/install paths."""
