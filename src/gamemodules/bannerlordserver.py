...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Bannerlord server", int),
            ArgSpec("DIR", "The directory to install Bannerlord in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Bannerlord dedicated server to the latest version.",
    "restart": "Restart the Bannerlord dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="Bannerlord.DedicatedServer"):
    ...
    server.data.setdefault("queryport", "7210")
    server.data.setdefault("game_type", "CustomBattle")
    server.data.setdefault("scene", "mp_sergeant_battle")
    server.data.setdefault("maxplayers", "32")
    server.data.setdefault("backupfiles", ["Configs", "Modules"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["Configs", "Modules"]}},
            "schedule": [("default", 0, "days")],
        }

    if port is None:
        port = server.data.get("port", 7210)
    if ask:
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Bannerlord server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Bannerlord server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Bannerlord server files and optionally restart the server."""
...
def checkvalue(server, key, *value):
    ...
    if key[0] in ("port", "queryport", "maxplayers"):
        return int(value[0])
    if key[0] in ("game_type", "scene", "exe_name", "dir"):
        return str(value[0])
