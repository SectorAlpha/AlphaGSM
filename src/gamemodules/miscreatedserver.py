...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Miscreated server", int),
            ArgSpec("DIR", "The directory to install Miscreated in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Miscreated dedicated server to the latest version.",
    "restart": "Restart the Miscreated dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="Bin64_dedicated/MiscreatedServer.exe"):
    ...
    server.data.setdefault("startmap", "islands")
    server.data.setdefault("gameserverid", "100")
    server.data.setdefault("servername", "AlphaGSM %s" % (server.name,))
    server.data.setdefault("backupfiles", ["data", "cfg", "Bin64_dedicated"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["data", "cfg"]}},
            "schedule": [("default", 0, "days")],
        }

    if port is None:
        port = server.data.get("port", 64090)
    if ask:
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Miscreated server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Miscreated server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Miscreated server files and optionally restart the server."""
