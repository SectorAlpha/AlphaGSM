...
def _assert_required_mod_content(install_dir):
    """Raise when the BrainBread mod payload has not been staged locally."""

    required_map = os.path.join(install_dir, "brainbread", "maps", "bb_chp4_slaywatch.bsp")
    if os.path.isfile(required_map):
        return
    gamemodule_common.raise_byo_requirement(
        "bbserver",
        "an owned BrainBread mod content tree",
        actions=(
            "Copy the complete BrainBread mod tree into <install_dir>/brainbread/",
            "Make sure <install_dir>/brainbread/maps/bb_chp4_slaywatch.bsp exists after staging the content",
...
def install(server):
    """Install the HLDS base files, then require staged BrainBread content."""
...
def get_start_command(server):
    """Build the start command after validating BrainBread content."""
