...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the VEIN server", int),
...
def configure(server, ask, port=None, dir=None, *, exe_name="VeinServer.sh"):
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install VEIN in: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the VEIN server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the VEIN server files and optionally restart the server."""
