...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Stationeers server", int),
            ArgSpec("DIR", "The directory to install Stationeers in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Stationeers dedicated server to the latest version.",
    "restart": "Restart the Stationeers dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="rocketstation_DedicatedServer.x86_64"):
    ...
    server.data.setdefault("worldname", "Space")
    server.data.setdefault("savename", server.name)
    server.data.setdefault("servername", "AlphaGSM %s" % (server.name,))
    server.data.setdefault("serverpassword", "")
    server.data.setdefault("updateport", "27015")
    server.data.setdefault("maxplayers", "10")
    server.data.setdefault("autosave", "true")
    server.data.setdefault("saveinterval", "300")
    server.data.setdefault("upnp", "false")
    server.data.setdefault("backupfiles", ["saves"])
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Stationeers server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Stationeers server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Stationeers server files and optionally restart the server."""
...
def restart(server):
    """Restart the Stationeers server."""
...
def checkvalue(server, key, *value):
    ...
    if key[0] in ("port", "updateport", "saveinterval", "maxplayers"):
        return int(value[0])
    if key[0] in (
        "worldname",
        "savename",
