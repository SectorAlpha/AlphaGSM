...
"""
SCPSL_MOD_CACHE_DIRNAME = "scpslserver"
SCPSL_LABAPI_ALLOWED_DESTINATIONS = ("plugins", "dependencies")
SCPSL_LABAPI_BARE_PLUGIN_DESTINATION = "plugins/global"
...
def load_scpsl_curated_registry(server=None):
    """Load the checked-in SCP:SL plugin registry or an override file."""

    del server
    override = os.environ.get("ALPHAGSM_SCPSL_CURATED_MODS_PATH")
...
def _scpsl_labapi_root(server):
    return os.path.join(_scpsl_config_root(server), "LabAPI")


MOD_SUPPORT = build_plugin_jar_mod_support(
    game_label="SCP:SL LabAPI",
    curated_registry_loader=load_scpsl_curated_registry,
    cache_namespace=SCPSL_MOD_CACHE_DIRNAME,
    download_to_cache_getter=lambda: download_to_cache,
    resolve_direct_url_entry_getter=lambda: resolve_direct_url_entry,
    allowed_destinations=SCPSL_LABAPI_ALLOWED_DESTINATIONS,
    direct_url_suffixes={
        ".dll": "dll",
        ".zip": "zip",
        ".7z": "7z",
    ...
    direct_url_filename_description="a plugin .dll or supported archive filename",
    direct_url_requirement_description="a LabAPI plugin URL",
    source_identifier_description="plugin family, direct plugin URL, or Mod DB page URL",
    single_file_destinations={"dll": SCPSL_LABAPI_BARE_PLUGIN_DESTINATION},
    bare_archive_destination=SCPSL_LABAPI_BARE_PLUGIN_DESTINATION,
    bare_archive_file_suffixes=(".dll", ".pdb", ".yml", ".yaml", ".json"),
    install_root_getter=_scpsl_labapi_root,
)
commands = commands + MOD_SUPPORT.commands
