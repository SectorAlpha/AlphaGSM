...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Last Oasis server", int),
            ArgSpec("DIR", "The directory to install Last Oasis in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Last Oasis dedicated server to the latest version.",
    "restart": "Restart the Last Oasis dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="LastOasisServer.x86_64"):
    ...
    server.data.setdefault("queryport", "15001")
    ...
    if port is None:
        port = server.data.get("port", 15000)
    if ask:
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Last Oasis server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Last Oasis server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Last Oasis server files and optionally restart the server."""
...
def get_start_command(server):
    ...
    return (
        [
            ...
            "-worldname=%s" % (server.data["worldname"],),
