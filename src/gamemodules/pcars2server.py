...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Project CARS 2 server", int),
            ArgSpec("DIR", "The directory to install Project CARS 2 in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Project CARS 2 dedicated server to the latest version.",
    "restart": "Restart the Project CARS 2 dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="DedicatedServerCmd"):
    ...
    server.data.setdefault("backupfiles", ["server.cfg", "lua"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["server.cfg", "lua"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Project CARS 2 server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Project CARS 2 server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Project CARS 2 server files and optionally restart the server."""
...
def restart(server):
    """Restart the Project CARS 2 server."""
