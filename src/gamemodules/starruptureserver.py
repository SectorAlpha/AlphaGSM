...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the StarRupture server", int),
            ArgSpec("DIR", "The directory to install StarRupture in", str),
        ),
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the StarRupture dedicated server to the latest version.",
    "restart": "Restart the StarRupture dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(
    ...
    dir=None,
    *,
    exe_name="StarRuptureServerEOS.exe",
):
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the StarRupture server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the StarRupture server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the StarRupture server files and optionally restart the server."""
...
def restart(server):
    """Restart the StarRupture server."""
