...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Night of the Dead server", int),
            ArgSpec("DIR", "The directory to install Night of the Dead in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Night of the Dead dedicated server to the latest version.",
    "restart": "Restart the Night of the Dead dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="LFServer.exe"):
    ...
    server.data.setdefault("backupfiles", ["LF/Saved", "ServerSettings.ini"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["LF/Saved", "ServerSettings.ini"]}},
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Night of the Dead server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Night of the Dead server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Night of the Dead server files and optionally restart the server."""
...
def restart(server):
    """Restart the Night of the Dead server."""
