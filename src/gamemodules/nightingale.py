...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Nightingale server", int),
            ArgSpec("DIR", "The directory to install Nightingale in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Nightingale dedicated server to the latest version.",
    "restart": "Restart the Nightingale dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="NWXServer.sh"):
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Nightingale server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Nightingale server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Nightingale server files and optionally restart the server."""
...
def restart(server):
    """Restart the Nightingale server."""
...
def checkvalue(server, key, *value):
    ...
    if key[0] == "port":
        return int(value[0])
    if key[0] in ("savegame", "exe_name", "dir"):
        return str(value[0])
