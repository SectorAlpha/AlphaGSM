...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ...
            ArgSpec("DIR", "The directory to install Beasts of Bermuda in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Beasts of Bermuda dedicated server to the latest version.",
    "restart": "Restart the Beasts of Bermuda dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="BeastsOfBermudaServer.sh"):
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Beasts of Bermuda server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Beasts of Bermuda server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Beasts of Bermuda server files and optionally restart the server."""
...
def restart(server):
    """Restart the Beasts of Bermuda server."""
...
def checkvalue(server, key, *value):
    ...
    if key[0] in ("port", "queryport"):
        return int(value[0])
    if key[0] in ("servername", "worldname", "password", "exe_name", "dir"):
        return str(value[0])
