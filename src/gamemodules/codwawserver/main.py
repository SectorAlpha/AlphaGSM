...
command_descriptions = {
    "mod": "Manage Call of Duty: World at War pk3 content from direct archive URLs or direct pk3 URLs into the active moddir directory."
}
command_functions = {}
max_stop_wait = 1
...
def ensure_mod_state(server):
    """Seed the COD: World at War mod desired-state shape and return it."""
...
def _ensure_moddir_backup(server):
    ...
    backupfiles = list(server.data.setdefault("backupfiles", ["main", "mods"]))
    ...
    default_profile = backup.setdefault("profiles", {}).setdefault("default", {"targets": ["main", "mods"]})
    targets = list(default_profile.setdefault("targets", ["main", "mods"]))
...
def _cache_root(server) -> Path:
    return Path(server.data["dir"]) / ".alphagsm" / "mods" / CODWAW_MOD_CACHE_DIRNAME
...
def _desired_record(server, resolved_id: str) -> dict:
    for entry in ensure_mod_state(server)["desired"]["url"]:
        if entry.get("resolved_id") == resolved_id:
            return entry
    raise ModSupportError(f"Missing Call of Duty: World at War desired-state metadata for '{resolved_id}'")
    ...
    if archive_type in ("zip", "7z", "tar"):
        ...
        if install_root is None:
            raise ModSupportError(
                f"No Call of Duty: World at War pk3 content was found in the downloaded payload; expected {moddir}/<name>.pk3 or bare .pk3 files"
            )
    else:
...
def codwaw_mod_command(server, action, source=None, identifier=None, extra=None, **kwargs):
    """Handle the COD: World at War `mod` command desired-state subcommands."""
    ...
    if action == "add" and source == "url":
        if not identifier:
            raise ServerError("Call of Duty: World at War mod add url requires a pk3 archive URL or direct pk3 URL")
        resolved = resolve_direct_url_entry(
            identifier,
            filename=extra,
            allowed_suffixes=CODWAW_ALLOWED_MOD_SUFFIXES,
            entry_label="Call of Duty: World at War mod URL entries",
        ...
        for entry in mods["desired"]["url"]:
            if entry.get("requested_id") == resolved["requested_id"]:
                raise ServerError(
                    f"Call of Duty: World at War content '{resolved['requested_id']}' is already present in desired state"
                )
    ...
    if action == "remove" and source == "url":
        if not identifier:
            raise ServerError("Call of Duty: World at War mod remove url requires the original content URL")
        existing = mods["desired"]["url"]
        updated = [entry for entry in existing if entry.get("requested_id") != identifier]
        if len(updated) == len(existing):
            raise ServerError(f"Call of Duty: World at War content '{identifier}' is not present in desired state")
        mods["desired"]["url"] = updated
        _save_data(server)
        return
    raise ServerError("Unsupported Call of Duty: World at War mod command")


command_functions["mod"] = codwaw_mod_command
