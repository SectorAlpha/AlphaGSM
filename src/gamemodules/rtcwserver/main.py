...
RTCW_MOD_CACHE_DIRNAME = "rtcwserver"
RTCW_ALLOWED_MOD_SUFFIXES = {
    ".7z": "7z",
    ".pk3": "pk3",
    ".tar.gz": "tar",
    ".tbz2": "tar",
...
command_descriptions = {
    "mod": "Manage RTCW pk3 content from direct archive URLs or direct pk3 URLs into the active fs_game directory."
}
command_functions = {}
max_stop_wait = 1
...
def ensure_mod_state(server):
    """Seed the RTCW mod desired-state shape and return it."""
...
def _fs_game(server) -> str:
    return str(server.data.get("fs_game") or "main")
...
def _cache_root(server) -> Path:
    return Path(server.data["dir"]) / ".alphagsm" / "mods" / RTCW_MOD_CACHE_DIRNAME
...
def _desired_record(server, resolved_id: str) -> dict:
    for entry in ensure_mod_state(server)["desired"]["url"]:
        if entry.get("resolved_id") == resolved_id:
            return entry
    raise ModSupportError(f"Missing RTCW desired-state metadata for '{resolved_id}'")
    ...
    if archive_type in ("zip", "7z", "tar"):
        ...
        if install_root is None:
            raise ModSupportError(
                f"No RTCW pk3 content was found in the downloaded payload; expected {fs_game}/<name>.pk3 or bare .pk3 files"
            )
    else:
...
def rtcw_mod_command(server, action, source=None, identifier=None, extra=None, **kwargs):
    """Handle the RTCW `mod` command desired-state subcommands."""
    ...
    if action == "add" and source == "url":
        if not identifier:
            raise ServerError("RTCW mod add url requires a pk3 archive URL or direct pk3 URL")
        resolved = resolve_direct_url_entry(
            identifier,
            filename=extra,
            allowed_suffixes=RTCW_ALLOWED_MOD_SUFFIXES,
            entry_label="RTCW mod URL entries",
        ...
        for entry in mods["desired"]["url"]:
            if entry.get("requested_id") == resolved["requested_id"]:
                raise ServerError(
                    f"RTCW content '{resolved['requested_id']}' is already present in desired state"
                )
    ...
    if action == "remove" and source == "url":
        if not identifier:
            raise ServerError("RTCW mod remove url requires the original content URL")
        existing = mods["desired"]["url"]
        updated = [entry for entry in existing if entry.get("requested_id") != identifier]
        if len(updated) == len(existing):
            raise ServerError(f"RTCW content '{identifier}' is not present in desired state")
        mods["desired"]["url"] = updated
        _save_data(server)
        return
    raise ServerError("Unsupported RTCW mod command")


command_functions["mod"] = rtcw_mod_command
...
def sync_server_config(server):
    ...
    _ensure_fs_game_backup(server)
    fs_game = server.data.get("fs_game", "main")
