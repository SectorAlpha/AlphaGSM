"""The Specialists-specific lifecycle, configuration, and update helpers."""
...
MODULE = define_valve_server_module(
    game_name='The Specialists',
    engine='goldsrc',
    steam_app_id=90,
    game_dir='ts',
    executable='hlds_run',
    default_map='ts_neobaroque',
    max_players=32,
    port=27015,
    client_port=27005,
    sourcetv_port=None,
...
def _assert_required_mod_content(install_dir):
    """Raise when The Specialists mod payload has not been staged locally."""

    required_map = os.path.join(install_dir, "ts", "maps", "ts_neobaroque.bsp")
    if os.path.isfile(required_map):
        return
    gamemodule_common.raise_byo_requirement(
        "tsserver",
        "an owned The Specialists mod content tree",
        actions=(
            "Copy the complete The Specialists mod tree into <install_dir>/ts/",
            "Make sure <install_dir>/ts/maps/ts_neobaroque.bsp exists after staging the content",
...
def install(server):
    """Install the HLDS base files, then require staged The Specialists content."""
...
def get_start_command(server):
    """Build the start command after validating The Specialists content."""
