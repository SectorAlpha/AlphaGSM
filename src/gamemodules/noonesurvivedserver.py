...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the No One Survived server", int),
            ArgSpec("DIR", "The directory to install No One Survived in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the No One Survived dedicated server to the latest version.",
    "restart": "Restart the No One Survived dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="WRSHServer.exe"):
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the No One Survived server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the No One Survived server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the No One Survived server files and optionally restart the server."""
...
def restart(server):
    """Restart the No One Survived server."""
...
def status(server, verbose):
    """Detailed No One Survived status is not implemented yet."""
