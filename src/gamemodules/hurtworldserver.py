...
command_args = {
    "setup": CmdSpec(
        optionalarguments=(
            ArgSpec("PORT", "The game port to use for the Hurtworld server", int),
            ArgSpec("DIR", "The directory to install Hurtworld in", str),
        )
    ),
    "update": CmdSpec(
        options=(
...
command_descriptions = {
    "update": "Update the Hurtworld dedicated server to the latest version.",
    "restart": "Restart the Hurtworld dedicated server.",
}
command_functions = {}
max_stop_wait = 1
...
def configure(server, ask, port=None, dir=None, *, exe_name="HurtworldDedicated"):
    ...
    server.data.setdefault("queryport", "12872")
    server.data.setdefault("maxplayers", "50")
    server.data.setdefault("worldname", server.name)
    server.data.setdefault("backupfiles", ["Save", "hurtworld.cfg"])
    if "backup" not in server.data:
        server.data["backup"] = {
            "profiles": {"default": {"targets": ["Save", "hurtworld.cfg"]}},
            "schedule": [("default", 0, "days")],
        }

    if port is None:
        port = server.data.get("port", 12871)
    if ask:
    ...
    if dir is None:
        ...
        if ask:
            inp = input("Where would you like to install the Hurtworld server: [%s] " % (dir,)).strip()
            if inp:
                dir = inp
...
def install(server):
    """Download the Hurtworld server files via SteamCMD."""
...
def update(server, validate=False, restart=False):
    """Update the Hurtworld server files and optionally restart the server."""
