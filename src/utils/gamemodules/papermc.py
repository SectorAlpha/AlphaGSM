"""Helpers for PaperMC-family downloads (moved from src/gamemodules)."""
...
def _get_download_url(build):
    ...
    preferred_key = next((key for key in downloads if key.endswith(":default")), None)
...
def resolve_download(project, version=None):
    ...
    for candidate in candidate_versions:
        builds = _read_json("%s/projects/%s/versions/%s/builds" % (PAPERMC_API_ROOT, project, candidate))
        for build in builds:
