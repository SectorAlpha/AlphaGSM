...
def parse(inargs, cmdspec):
            ...
            if len(arg) > 1 and arg[1] == "-":  # this argument is a long option
                ...
                if opt.hasargument():
                    ...
                    try:
                        ...
                        raise OptionError(
                            "Argument isn't of the right format for option '--"
                            + arg[2:]
                            + "'",
                            ex,
                        )
    ...
    outargs = [
        _convertarg(arg, spec)
        for arg, spec in zip(outargs, _replast(cmdspec.allarguments))
    ]
...
def _convertarg(arg, spec):
    ...
    try:
        ...
        raise OptionError(
            "Argument isn't of the right format for '" + spec.name + "'", ex
        )
...
def shorthelp(cmd, cmddesc, cmdspec, file=sys.stderr):
    ...
    print(
        textwrap.fill(s, 80, initial_indent="  ", subsequent_indent="        "),
        file=file,
    )
...
def longhelp(cmd, cmddesc, cmdspec, file=sys.stderr):
    ...
    if cmdspec.hasoptions():
        ...
        for opt in cmdspec.options:
            s = ", ".join(
                chain(
                    ("-" + s for s in opt.shortforms), ("--" + l for l in opt.longforms)
                )
            )
            if opt.hasargument():
                s += " " + opt.argument + " "
