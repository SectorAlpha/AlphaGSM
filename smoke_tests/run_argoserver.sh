...
echo "Smoke test for argoserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itargoserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/argoserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-argoserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-argoserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-argoserver-IT#
keeplogs = 1
EOF
...
run_alphagsm "$SERVER_NAME" create argoserver
