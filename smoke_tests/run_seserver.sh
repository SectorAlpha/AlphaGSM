...
echo "Smoke test for seserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itseserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/seserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-seserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-seserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-seserver-IT#
keeplogs = 1
EOF
...
run_alphagsm "$SERVER_NAME" create seserver
