...
def test_reconcile_rejects_unresolved_desired_entries():
    desired_entry = DesiredModEntry(
        source_type="curated",
        requested_id="sourcemod",
        resolved_id=None,
    ...
    with pytest.raises(ModSupportError, match="sourcemod"):
        reconcile_mod_state(
            desired=[desired_entry],
            installed=[],
            install_entry=install_entry,
...
def test_reconcile_returns_installed_state_in_desired_order_for_mixed_entries():
    desired_keep = DesiredModEntry(
        source_type="curated",
        requested_id="sourcemod",
        resolved_id="sourcemod.stable",
    )
    desired_install = DesiredModEntry(
        source_type="curated",
        requested_id="metamod",
        resolved_id="metamod.stable",
    )
    kept_entry = InstalledModEntry(
        source_type="curated",
    ...
    stale_entry = InstalledModEntry(
        source_type="curated",
        resolved_id="unused.stable",
        installed_files=["tf/addons/unused.vdf"],
    )
    installed_entry = InstalledModEntry(
        source_type="curated",
        resolved_id="metamod.stable",
    ...
    result = reconcile_mod_state(
        desired=[desired_install, desired_keep],
        installed=[stale_entry, kept_entry],
        install_entry=install_entry,
        remove_entry=remove_entry,
    )

    assert result == [installed_entry, kept_entry]
    assert install_calls == [desired_install]
    assert remove_calls == [stale_entry]
