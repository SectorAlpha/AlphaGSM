"""Focused QuakeWorld pak mod tests."""
...
def test_qw_mod_state_seeds_defaults(tmp_path):
...
def test_qw_mod_add_url_records_desired_state(tmp_path):
    server = DummyServer(tmp_path / "server")
...
def test_qw_mod_apply_installs_archive_content_into_qw_root(tmp_path, monkeypatch):
    ...
    _write_zip(
        archive_path,
        {
            "qw/pak1.pak": "pak-data",
            "README.txt": "ignored",
        },
    )
    monkeypatch.setattr(mod._main, "download_to_cache", _fake_download_factory(archive_path))

    mod.qw_mod_command(server, "add", "url", "https://example.com/payload.zip")
    mod.qw_mod_command(server, "apply")

    assert (server_root / "qw" / "pak1.pak").read_text() == "pak-data"
    assert server.data["mods"]["last_apply"] == "success"
    assert "qw" in server.data["backupfiles"]
    assert "qw" in server.data["backup"]["profiles"]["default"]["targets"]


def test_qw_mod_apply_installs_bare_pak_archive_into_qw_root(tmp_path, monkeypatch):
    server_root = tmp_path / "server"
    server_root.mkdir()
    server = DummyServer(server_root)
    ...
    mod.qw_mod_command(server, "add", "url", "https://example.com/maps.zip")
    mod.qw_mod_command(server, "apply")

    assert (server_root / "qw" / "pak2.pak").read_text() == "pak-data"


def test_qw_mod_apply_rejects_non_pak_archive(tmp_path, monkeypatch):
    server_root = tmp_path / "server"
    server_root.mkdir()
    server = DummyServer(server_root)
    archive_path = tmp_path / "bad.zip"
    _write_zip(archive_path, {"qw/readme.txt": "bad"})
    monkeypatch.setattr(mod._main, "download_to_cache", _fake_download_factory(archive_path))

    mod.qw_mod_command(server, "add", "url", "https://example.com/bad.zip")

    with pytest.raises(ServerError):
        mod.qw_mod_command(server, "apply")

    assert server.data["mods"]["errors"]
    assert not (server_root / "qw" / "readme.txt").exists()


def test_qw_mod_cleanup_removes_only_owned_files(tmp_path, monkeypatch):
    server_root = tmp_path / "server"
    server_root.mkdir()
    server = DummyServer(server_root)
    archive_path = tmp_path / "payload.zip"
    _write_zip(archive_path, {"qw/pak-owned.pak": "owned"})
    monkeypatch.setattr(mod._main, "download_to_cache", _fake_download_factory(archive_path))

    mod.qw_mod_command(server, "add", "url", "https://example.com/payload.zip")
    mod.qw_mod_command(server, "apply")
    unmanaged_path = server_root / "qw" / "pak-unmanaged.pak"
    unmanaged_path.parent.mkdir(parents=True, exist_ok=True)
    unmanaged_path.write_text("keep")

    mod.qw_mod_command(server, "cleanup")

    assert not (server_root / "qw" / "pak-owned.pak").exists()
