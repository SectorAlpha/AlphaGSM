"""Focused Call of Duty pk3 mod tests."""
...
def test_cod_mod_state_seeds_defaults(tmp_path):
...
def test_cod_mod_add_url_records_desired_state(tmp_path):
    server = DummyServer(tmp_path / "server")
...
def test_cod_mod_apply_installs_archive_content_into_active_moddir(tmp_path, monkeypatch):
    ...
    mod.cod_mod_command(server, "add", "url", "https://example.com/payload.zip")
    mod.cod_mod_command(server, "apply")
...
def test_cod_mod_apply_installs_bare_pk3_archive_into_default_moddir(tmp_path, monkeypatch):
    server_root = tmp_path / "server"
    server_root.mkdir()
    server = DummyServer(server_root)
    ...
    mod.cod_mod_command(server, "add", "url", "https://example.com/maps.zip")
    mod.cod_mod_command(server, "apply")

    assert (server_root / "main" / "pak-custom.pk3").read_text() == "pk3-data"


def test_cod_mod_apply_rejects_non_pk3_archive(tmp_path, monkeypatch):
    server_root = tmp_path / "server"
    server_root.mkdir()
    server = DummyServer(server_root)
    ...
    mod.cod_mod_command(server, "add", "url", "https://example.com/bad.zip")
...
def test_cod_mod_cleanup_removes_only_owned_files(tmp_path, monkeypatch):
    server_root = tmp_path / "server"
    server_root.mkdir()
    server = DummyServer(server_root)
    archive_path = tmp_path / "payload.zip"
    ...
    mod.cod_mod_command(server, "cleanup")
