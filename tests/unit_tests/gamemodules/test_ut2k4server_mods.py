"""Focused UT2004 content-mod tests."""
...
def test_ut2k4_mod_add_url_records_desired_state(tmp_path):
    ...
    mod.ut2k4_mod_command(
        ...
        "add",
        "url",
        "https://example.com/DM-Test.ut2",
    )

    assert server.data["mods"]["desired"]["url"][0]["filename"] == "DM-Test.ut2"
    assert server.data["mods"]["desired"]["url"][0]["archive_type"] == "ut2"
...
def test_ut2k4_mod_apply_installs_archive_content(tmp_path, monkeypatch):
    ...
    mod.ut2k4_mod_command(server, "add", "url", "https://example.com/payload.zip")
    mod.ut2k4_mod_command(server, "apply")

    assert (server_root / "Maps" / "DM-Test.ut2").read_text() == "map-data"
...
def test_ut2k4_mod_apply_rejects_system_payload(tmp_path, monkeypatch):
    ...
    mod.ut2k4_mod_command(server, "add", "url", "https://example.com/bad.zip")
...
def test_ut2k4_mod_cleanup_removes_only_owned_files(tmp_path, monkeypatch):
    ...
    mod.ut2k4_mod_command(server, "add", "url", "https://example.com/payload.zip")
    mod.ut2k4_mod_command(server, "apply")
    unmanaged_path = server_root / "Maps" / "DM-Unmanaged.ut2"
    unmanaged_path.parent.mkdir(parents=True, exist_ok=True)
    unmanaged_path.write_text("keep-me")

    mod.ut2k4_mod_command(server, "cleanup")

    assert not (server_root / "Maps" / "DM-Owned.ut2").exists()
