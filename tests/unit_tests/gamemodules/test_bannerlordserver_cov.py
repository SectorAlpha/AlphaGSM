"""Full coverage tests for bannerlordserver."""
...
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock(), 'utils.steamcmd': MagicMock()}):
    import gamemodules.bannerlordserver as mod
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["7211", str(tmp_path / 'custom')])
...
def test_checkvalue_game_type():
    server = DummyServer()
    result = mod.checkvalue(server, ("game_type",), "/test/value")
    assert result == "/test/value"


def test_checkvalue_scene():
    server = DummyServer()
    result = mod.checkvalue(server, ("scene",), "/test/value")
    assert result == "/test/value"
