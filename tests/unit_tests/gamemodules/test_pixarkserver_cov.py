"""Full coverage tests for pixarkserver."""
...
sys.modules.pop('gamemodules.pixarkserver', None)
...
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock(), 'utils.steamcmd': MagicMock(), 'utils.proton': _proton_mock}):
    import gamemodules.pixarkserver as mod
...
def test_install(tmp_path):
    server = DummyServer()
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "PixARKServer.exe"
    server.data["Steam_AppID"] = 824360
...
def test_get_start_command(tmp_path, monkeypatch):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "PixARKServer.exe"
    (tmp_path / "PixARKServer.exe").write_text("")
    server.data["map"] = "test"
    server.data["maxplayers"] = 27015
    server.data["port"] = 27015
    server.data["queryport"] = 27015
...
def test_setting_schema_exposes_pixark_launch_formats():
