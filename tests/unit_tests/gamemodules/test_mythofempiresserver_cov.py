"""Full coverage tests for mythofempiresserver."""
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["12889", str(tmp_path / 'custom')])
...
def test_install(tmp_path):
    server = DummyServer()
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "MOE/Binaries/Win64/MOEServer.exe"
    server.data["Steam_AppID"] = 1794810
...
def test_get_start_command(tmp_path, monkeypatch):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "MOE/Binaries/Win64/MOEServer.exe"
    exe_path = tmp_path / "MOE/Binaries/Win64/MOEServer.exe"
