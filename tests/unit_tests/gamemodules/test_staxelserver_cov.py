"""Full coverage tests for staxelserver."""
...
sys.modules.pop('gamemodules.staxelserver', None)
...
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock(), 'utils.steamcmd': MagicMock(), 'utils.proton': _proton_mock}):
    import gamemodules.staxelserver as mod
...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=25565, dir=str(tmp_path))
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["25566", str(tmp_path / 'custom')])
...
def test_install(tmp_path):
    server = DummyServer()
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "bin/Staxel.ServerWizard.exe"
    server.data["Steam_AppID"] = 755170
...
def test_get_start_command(tmp_path, monkeypatch):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "bin/Staxel.ServerWizard.exe"
    exe_path = tmp_path / "bin/Staxel.ServerWizard.exe"
