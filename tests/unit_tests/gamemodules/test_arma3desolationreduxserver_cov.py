"""Full coverage tests for arma3desolationreduxserver."""
...
sys.modules.pop('gamemodules.arma3desolationreduxserver', None)
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock(), 'utils.steamcmd': MagicMock()}):
    import gamemodules.arma3desolationreduxserver as mod
...
def test_configure_ask_defaults(tmp_path, monkeypatch):
    ...
    server.data["mission"] = "test"
    server.data["mod"] = "test"
    server.data["profilesdir"] = "test"
    server.data["servermod"] = "test"
...
def test_get_start_command(tmp_path):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "arma3server_x64"
    (tmp_path / "arma3server_x64").write_text("")
    server.data["configfile"] = "test"
    server.data["mission"] = "test"
    server.data["mod"] = "test"
...
def test_checkvalue_mod():
    server = DummyServer()
    result = mod.checkvalue(server, ("mod",), "/test/value")
    assert result == "/test/value"


def test_checkvalue_servermod():
    server = DummyServer()
    result = mod.checkvalue(server, ("servermod",), "/test/value")
    assert result == "/test/value"


def test_checkvalue_mission():
    server = DummyServer()
    result = mod.checkvalue(server, ("mission",), "/test/value")
    assert result == "/test/value"
