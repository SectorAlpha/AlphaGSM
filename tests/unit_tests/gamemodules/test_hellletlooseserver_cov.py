"""Full coverage tests for hellletlooseserver."""
...
sys.modules.pop('gamemodules.hellletlooseserver', None)
...
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock(), 'utils.steamcmd': MagicMock(), 'utils.proton': _proton_mock}):
    import gamemodules.hellletlooseserver as mod
...
def test_install(tmp_path):
    server = DummyServer()
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "HLLServer.exe"
    server.data["Steam_AppID"] = 822500
...
def test_get_start_command(tmp_path, monkeypatch):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "HLLServer.exe"
    (tmp_path / "HLLServer.exe").write_text("")
    server.data["startmap"] = "test"
