"""Full coverage tests for dayzarma2epochserver."""
...
sys.modules.pop('gamemodules.dayzarma2epochserver', None)
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock(), 'utils.steamcmd': MagicMock()}):
    import gamemodules.dayzarma2epochserver as mod
...
def test_install(tmp_path):
    server = DummyServer()
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "arma2oaserver"
    server.data["Steam_AppID"] = 33935
...
def test_get_start_command(tmp_path):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "arma2oaserver"
    (tmp_path / "arma2oaserver").write_text("")
    server.data["configfile"] = "test"
    server.data["mod"] = "test"
