"""Full coverage tests for necserver."""
...
sys.modules.pop('gamemodules.necserver', None)
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock(), 'utils.steamcmd': MagicMock()}):
    import gamemodules.necserver as mod
...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=14159, dir=str(tmp_path))
    assert server.data['port'] == 14159


def test_configure_ask_defaults(tmp_path, monkeypatch):
    monkeypatch.setattr("builtins.input", lambda prompt: "")
    server = DummyServer()
    server.data["port"] = 14159
    server.data["dir"] = str(tmp_path) + "/"
    server.data["Steam_AppID"] = "test"
    server.data["Steam_anonymous_login_possible"] = "test"
    server.data["javapath"] = "test"
    server.data["servername"] = "test"
    server.data["slots"] = 27015
    server.data["world"] = "test"
    mod.configure(server, ask=True)


def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["14160", str(tmp_path / 'custom')])
...
def test_checkvalue_slots():
    server = DummyServer()
    result = mod.checkvalue(server, ("slots",), "12345")
    assert result == 12345
