"""Full coverage tests for kf2server."""
...
def test_install(tmp_path):
    server = DummyServer()
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "Binaries/Win64/KFGameSteamServer.bin.x86_64"
    server.data["Steam_AppID"] = 232130
...
def test_get_start_command(tmp_path):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "Binaries/Win64/KFGameSteamServer.bin.x86_64"
    exe_path = tmp_path / "Binaries/Win64/KFGameSteamServer.bin.x86_64"
...
def test_checkvalue_configsubdir():
    server = DummyServer()
    result = mod.checkvalue(server, ("configsubdir",), "/test/value")
    assert result == "/test/value"
