"""Full coverage tests for gtafivemserver."""
...
sys.modules.pop('gamemodules.gtafivemserver', None)
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.archive_install': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock()}):
    import gamemodules.gtafivemserver as mod
...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=30120, dir=str(tmp_path), url="https://example.com/test.zip", download_name="test.zip", version="1.0")
    assert server.data['port'] == 30120

def test_configure_ask_defaults(tmp_path, monkeypatch):
    monkeypatch.setattr("builtins.input", lambda prompt: "")
    server = DummyServer()
    server.data["port"] = 30120
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["30121", str(tmp_path / 'custom'), "https://example.com/new.tar.gz"])
...
def test_configure_resolves_download(tmp_path):
    server = DummyServer()
    with patch.object(mod, 'resolve_download', return_value=('7777', 'https://example.com/fx.tar.xz')):
        mod.configure(server, ask=False, port=30120, dir=str(tmp_path))
    assert server.data['url'] == 'https://example.com/fx.tar.xz'
...
def test_install_resolves_download(tmp_path):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "run.sh"
    server.data["download_name"] = "fx.tar.gz"
    with patch.object(mod, 'resolve_download', return_value=('7777', 'https://example.com/fx.tar.xz')):
        mod.install(server)
    assert server.data['url'] == 'https://example.com/fx.tar.xz'
    assert server.data['version'] == '7777'
