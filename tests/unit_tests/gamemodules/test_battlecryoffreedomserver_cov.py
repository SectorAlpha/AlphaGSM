"""Full coverage tests for battlecryoffreedomserver."""
...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=8262, dir=str(tmp_path))
    assert server.data['port'] == 8262


def test_configure_ask_defaults(tmp_path, monkeypatch):
    monkeypatch.setattr("builtins.input", lambda prompt: "")
    server = DummyServer()
    server.data["port"] = 8262
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["8263", str(tmp_path / 'custom')])
...
def test_install(tmp_path):
    server = DummyServer()
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "BCoF.exe"
    server.data["Steam_AppID"] = 1362540
