"""Full coverage tests for q4server."""
...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=28004, dir=str(tmp_path), url="https://example.com/test.zip", download_name="test.zip")
    assert server.data['port'] == 28004

def test_configure_ask_defaults(tmp_path, monkeypatch):
    monkeypatch.setattr("builtins.input", lambda prompt: "")
    server = DummyServer()
    server.data["port"] = 28004
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["28005", str(tmp_path / 'custom'), "https://example.com/new.tar.gz"])
...
def test_setting_schema_exposes_q4_launch_tokens():
