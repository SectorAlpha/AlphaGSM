"""Full coverage tests for bf1942server."""
...
sys.modules.pop('gamemodules.bf1942server', None)
...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=14567, dir=str(tmp_path), url="https://example.com/test.zip", download_name="test.zip")
    assert server.data['port'] == 14567


def test_configure_ask_defaults(tmp_path, monkeypatch):
    monkeypatch.setattr("builtins.input", lambda prompt: "")
    server = DummyServer()
    server.data["port"] = 14567
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["14568", str(tmp_path / 'custom'), "https://example.com/new.tar.gz"])
...
def test_get_start_command(tmp_path):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "bf1942_lnxded"
    (tmp_path / "bf1942_lnxded").write_text("")
