...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=7777, dir=str(tmp_path), url="https://example.com/test.zip", download_name="test.zip", version="1.0")
...
def test_configure_ask_defaults(tmp_path, monkeypatch):
    ...
    server.data["auth_token"] = "test"
    server.data["beta_branch"] = "test"
    server.data["database"] = "test"
    server.data["maxplayers"] = 27015
    server.data["server_guid"] = "test"
    server.data["server_name"] = "test"
...
def test_get_start_command(tmp_path):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "PathOfTitansServer.sh"
    (tmp_path / "PathOfTitansServer.sh").write_text("")
    server.data["auth_token"] = "test"
    server.data["beta_branch"] = "test"
    server.data["database"] = "test"
    server.data["maxplayers"] = 27015
...
def test_checkvalue_server_name():
    server = DummyServer()
    result = mod.checkvalue(server, ("server_name",), "/test/value")
    assert result == "/test/value"

def test_checkvalue_server_guid():
    server = DummyServer()
    result = mod.checkvalue(server, ("server_guid",), "/test/value")
    assert result == "/test/value"
...
def test_checkvalue_beta_branch():
    server = DummyServer()
    result = mod.checkvalue(server, ("beta_branch",), "/test/value")
    assert result == "/test/value"

def test_checkvalue_auth_token():
    server = DummyServer()
    result = mod.checkvalue(server, ("auth_token",), "/test/value")
    assert result == "/test/value"
