...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=10578, dir=str(tmp_path), url="https://example.com/test.zip", download_name="test.zip", version="1.0")
    assert server.data['port'] == 10578

def test_configure_ask_defaults(tmp_path, monkeypatch):
    monkeypatch.setattr("builtins.input", lambda prompt: "")
    server = DummyServer()
    server.data["port"] = 10578
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["10579", str(tmp_path / 'custom'), "https://example.com/new.tar.gz"])
...
def test_get_start_command(tmp_path):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "SkyrimTogetherServer"
    (tmp_path / "SkyrimTogetherServer").write_text("")
