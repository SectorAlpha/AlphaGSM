"""Full coverage tests for subnauticaserver."""
...
sys.modules.pop('gamemodules.subnauticaserver', None)
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.archive_install': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock(), 'utils.github_releases': MagicMock()}):
    import gamemodules.subnauticaserver as mod
...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=11000, dir=str(tmp_path), url="https://example.com/test.zip", download_name="test.zip", version="1.0")
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["11001", str(tmp_path / 'custom'), "https://example.com/new.tar.gz"])
...
def test_install(tmp_path):
    server = DummyServer()
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "NitroxServer"
    server.data["url"] = "https://example.com/test.zip"
    server.data["download_name"] = "test.zip"
    server.data["version"] = "test"
    mod.install(server)
...
def test_get_start_command(tmp_path):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "NitroxServer"
    (tmp_path / "NitroxServer").write_text("")
