"""Full coverage tests for icarusserver."""
...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=17777, dir=str(tmp_path))
    assert server.data['port'] == 17777
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["17778", str(tmp_path / 'custom')])
...
def test_install(tmp_path):
    ...
    server.data["Steam_AppID"] = 2089300
...
def test_get_start_command(tmp_path, monkeypatch):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "IcarusServer.exe"
    (tmp_path / "IcarusServer.exe").write_text("")
