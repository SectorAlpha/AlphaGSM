...
sys.modules.pop('gamemodules.stationeersserver', None)
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock(), 'utils.steamcmd': MagicMock()}):
    import gamemodules.stationeersserver as mod
...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=27500, dir=str(tmp_path))
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["27501", str(tmp_path / 'custom')])
...
def test_install(tmp_path):
    ...
    server.data["Steam_AppID"] = 600760
...
def test_checkvalue_updateport():
    server = DummyServer()
    result = mod.checkvalue(server, ("updateport",), "12345")
    assert result == 12345


def test_checkvalue_saveinterval():
    server = DummyServer()
    result = mod.checkvalue(server, ("saveinterval",), "12345")
    assert result == 12345
...
def test_checkvalue_savename():
    server = DummyServer()
    result = mod.checkvalue(server, ("savename",), "/test/value")
    assert result == "/test/value"
...
def test_checkvalue_autosave():
    server = DummyServer()
    result = mod.checkvalue(server, ("autosave",), "/test/value")
    assert result == "/test/value"


def test_checkvalue_upnp():
    server = DummyServer()
    result = mod.checkvalue(server, ("upnp",), "/test/value")
    assert result == "/test/value"
