"""Full coverage tests for mxbikesserver."""
...
sys.modules.pop('gamemodules.mxbikesserver', None)
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.archive_install': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock()}):
    import gamemodules.mxbikesserver as mod
...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=54210, dir=str(tmp_path), url="https://example.com/test.zip", download_name="test.zip")
    assert server.data['port'] == 54210

def test_configure_ask_defaults(tmp_path, monkeypatch):
    monkeypatch.setattr("builtins.input", lambda prompt: "")
    server = DummyServer()
    server.data["port"] = 54210
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["54211", str(tmp_path / 'custom'), "https://example.com/new.tar.gz"])
...
def test_get_start_command(tmp_path):
    ...
    server.data["dir"] = str(tmp_path) + "/"
    server.data["exe_name"] = "mxbikes_dedicated"
    (tmp_path / "mxbikes_dedicated").write_text("")
