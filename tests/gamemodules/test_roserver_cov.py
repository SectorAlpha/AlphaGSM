"""Full coverage tests for roserver."""
...
sys.modules.pop('gamemodules.roserver', None)
with patch.dict('sys.modules', {'screen': MagicMock(), 'utils.backups': MagicMock(), 'utils.backups.backups': MagicMock(), 'utils.steamcmd': MagicMock()}):
    import gamemodules.roserver as mod
...
def test_configure_basic(tmp_path):
    server = DummyServer()
    mod.configure(server, ask=False, port=7757, dir=str(tmp_path))
    assert server.data['port'] == 7757


def test_configure_ask_defaults(tmp_path, monkeypatch):
    monkeypatch.setattr("builtins.input", lambda prompt: "")
    server = DummyServer()
    server.data["port"] = 7757
...
def test_configure_ask_custom(tmp_path, monkeypatch):
    inputs = iter(["7758", str(tmp_path / 'custom')])
...
def test_install(tmp_path):
    ...
    server.data["Steam_AppID"] = 223250
