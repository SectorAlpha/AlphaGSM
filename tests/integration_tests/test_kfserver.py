...
from gamemodules.kfserver import steam_app_id

pytestmark = [
    pytest.mark.integration,
    pytest.mark.skip(
        reason="ENABLED (AUTH): authenticate Steam or SteamCMD with an account entitled to Killing Floor dedicated server app 215360 before setup/start"
    ),
]
...
def test_kfserver_lifecycle(tmp_path):
    require_integration_opt_in()
    require_steamcmd_opt_in()
    ...
    # create
    run_and_assert_ok(env, server_name, "create", "kfserver")
