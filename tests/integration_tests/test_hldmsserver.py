"""Integration test for hldmsserver."""
...
from gamemodules.hldmsserver import steam_app_id
...
def _wait_for_info_protocol(env, server_name, expected_protocol, timeout_seconds):
    ...
    pytest.fail(
        f"hldmsserver info --json never returned protocol {expected_protocol!r} within {timeout_seconds}s: "
        f"last payload={last_data!r}"
    )


def _read_info_json(env, server_name):
...
def _assert_common_hldms_info(data):
    ...
    assert data.get("map") == "crossfire", f"Expected crossfire map: {data!r}"
    assert data.get("name") == "AlphaGSM Half-Life Deathmatch: Source", (
        f"Unexpected HLDMS name: {data!r}"
    )


def test_hldmsserver_lifecycle(tmp_path):
    require_integration_opt_in()
    require_steamcmd_opt_in()
    ...
    # create
    run_and_assert_ok(env, server_name, "create", "hldmsserver")
    ...
    server_cfg_path = install_dir / "hl1mp" / "cfg" / "server.cfg"
    ...
    try:
        ...
        _assert_common_hldms_info(_info_data)
    finally:
        # stop
