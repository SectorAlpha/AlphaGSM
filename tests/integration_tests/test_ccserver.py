...
from gamemodules.ccserver import steam_app_id
from utils.valve_server import detect_query_host

pytestmark = [
    pytest.mark.integration,
    pytest.mark.skip(
        reason="SteamCMD app 383410 anonymous install repeatedly returns state 0x202; dedicated server payload is not installable in CI"
    ),
]
...
def test_ccserver_lifecycle(tmp_path):
    require_integration_opt_in()
    require_steamcmd_opt_in()
    runtime_backend = os.environ.get(
        "ALPHAGSM_TEST_RUNTIME_BACKEND", default_runtime_backend()
    )
    module_name = "ccserver"
    require_command_for_runtime(
        "screen",
    ...
    # create
    run_and_assert_ok(env, server_name, "create", "ccserver")
