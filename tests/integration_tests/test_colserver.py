...
def test_colserver_lifecycle(tmp_path):
    ...
    server_name = ("itcol" + tmp_path.name.replace("_", "")[-10:])[:15]
    ...
    # create
    run_and_assert_ok(env, server_name, "create", "colserver")
    ...
    try:
        ...
        assert _info_data["protocol"] == "udp", f"Expected udp protocol in info JSON: {_info_data!r}"
    ...
    # verify stopped
    wait_for_generic_udp_closed("127.0.0.1", queryport, STOP_TIMEOUT)
