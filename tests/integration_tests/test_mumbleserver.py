...
def test_mumbleserver_lifecycle(tmp_path):
    ...
    # create
    run_and_assert_ok(env, server_name, "create", "mumbleserver")
    ...
    try:
        wait_for_tcp_open("127.0.0.1", port, START_TIMEOUT)
        ...
        assert _info_data.get("port") == port, (
            f"Expected tcp info to report the game port: {_info_data!r}"
        )
    finally:
        # stop
