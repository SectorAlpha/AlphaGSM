...
echo "Smoke test for citadelserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itcitadelser}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/citadelserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-citadelserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-citadelser-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-citadelser-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create citadelserver
