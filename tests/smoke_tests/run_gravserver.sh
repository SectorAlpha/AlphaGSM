...
echo "Smoke test for gravserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itgravserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/gravserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-gravserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-gravserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-gravserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create gravserver
