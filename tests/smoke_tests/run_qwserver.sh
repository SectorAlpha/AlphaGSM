...
echo "Smoke test for qwserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itqwserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/qwserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-qwserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-qwserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-qwserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create qwserver
