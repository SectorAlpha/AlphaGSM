...
echo "Smoke test for ecoserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itecoserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/ecoserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-ecoserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-ecoserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-ecoserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create ecoserver
