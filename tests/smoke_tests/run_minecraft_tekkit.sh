#\!/usr/bin/env bash
# ENABLED (BYO): supported with an operator-supplied direct archive URL or staged Tekkit jar.
echo "Smoke test for minecraft_tekkit is ENABLED (BYO) - set url to a direct Tekkit server archive URL or stage Tekkit.jar in <install_dir>/ before start"
exit 77
...
SERVER_NAME="${SERVER_NAME:-itminecraftt}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/minecraft-tekkit-server"
CONFIG_PATH="$WORK_DIR/alphagsm-minecraft-tekkit.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-minecraftt-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-minecraftt-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create minecraft.tekkit
