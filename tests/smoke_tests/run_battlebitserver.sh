...
echo "Smoke test for battlebitserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itbattlebits}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/battlebitserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-battlebitserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-battlebits-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-battlebits-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create battlebitserver
