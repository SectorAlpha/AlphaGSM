...
SERVER_NAME="${SERVER_NAME:-itl4dserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/l4dserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-l4dserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-l4dserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-l4dserver-IT#
keeplogs = 1

# Keep the smoke server awake for real A2S query and info checks.
[gamemodules.l4dserver.servercfg]
...
run_create_or_skip_disabled "$SERVER_NAME" create l4dserver
