...
SERVER_NAME="${SERVER_NAME:-itdodsserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/dodsserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-dodsserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-dodsserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-dodsserver-IT#
keeplogs = 1

# Keep the smoke server awake for real A2S query and info checks.
[gamemodules.dodsserver.servercfg]
...
run_create_or_skip_disabled "$SERVER_NAME" create dodsserver
