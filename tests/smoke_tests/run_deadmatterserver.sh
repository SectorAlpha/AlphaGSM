#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 1110990.
echo "Smoke test for deadmatterserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Dead Matter dedicated server app 1110990 before setup/start."
exit 77
...
SERVER_NAME="${SERVER_NAME:-itdeadmatter}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/deadmatterserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-deadmatterserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-deadmatter-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-deadmatter-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create deadmatterserver
