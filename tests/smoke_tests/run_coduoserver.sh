...
echo "Smoke test for coduoserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itcoduoserve}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/coduoserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-coduoserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-coduoserve-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-coduoserve-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create coduoserver
