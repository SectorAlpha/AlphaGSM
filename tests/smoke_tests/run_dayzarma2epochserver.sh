...
echo "Smoke test for dayzarma2epochserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Arma 2: Combined Operations dedicated server app 33935 before setup/start."
exit 0
...
SERVER_NAME="${SERVER_NAME:-itdayzarma2e}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/dayzarma2epochserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-dayzarma2epochserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-dayzarma2e-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-dayzarma2e-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create dayzarma2epochserver
