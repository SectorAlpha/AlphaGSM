...
echo "Smoke test for bobserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itbobserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/bobserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-bobserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-bobserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-bobserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create bobserver
