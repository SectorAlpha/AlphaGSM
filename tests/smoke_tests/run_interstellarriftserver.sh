#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 363360.
echo "Smoke test for interstellarriftserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Interstellar Rift dedicated server app 363360 before setup/start."
exit 0
...
SERVER_NAME="${SERVER_NAME:-itinterstell}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/interstellarriftserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-interstellarriftserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-interstell-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-interstell-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create interstellarriftserver
