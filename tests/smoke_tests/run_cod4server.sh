...
echo "Smoke test for cod4server is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itcod4server}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/cod4server-server"
CONFIG_PATH="$WORK_DIR/alphagsm-cod4server.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-cod4server-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-cod4server-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create cod4server
