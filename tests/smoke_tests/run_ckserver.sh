...
echo "Smoke test for ckserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itckserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/ckserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-ckserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-ckserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-ckserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create ckserver
