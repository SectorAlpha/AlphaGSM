#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 2690320.
echo "Smoke test for ducksideserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Duckside dedicated server app 2690320 before setup/start."
exit 0
...
SERVER_NAME="${SERVER_NAME:-itducksidese}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/ducksideserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-ducksideserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-ducksidese-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-ducksidese-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create ducksideserver
