...
echo "Smoke test for q4server is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itq4server}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/q4server-server"
CONFIG_PATH="$WORK_DIR/alphagsm-q4server.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-q4server-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-q4server-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create q4server
