...
SERVER_NAME="${SERVER_NAME:-itbmdmserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/bmdmserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-bmdmserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-bmdmserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-bmdmserver-IT#
keeplogs = 1

# Keep the smoke server awake for real A2S query and info checks.
[gamemodules.bmdmserver.servercfg]
...
run_create_or_skip_disabled "$SERVER_NAME" create bmdmserver
