...
echo "Smoke test for bbserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itbbserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/bbserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-bbserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-bbserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-bbserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create bbserver
