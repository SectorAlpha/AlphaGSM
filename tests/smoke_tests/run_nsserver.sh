...
echo "Smoke test for nsserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itnsserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/nsserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-nsserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-nsserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-nsserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create nsserver
