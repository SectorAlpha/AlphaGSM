...
SERVER_NAME="${SERVER_NAME:-itfofserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/fofserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-fofserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-fofserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-fofserver-IT#
keeplogs = 1

# Keep the smoke server awake for real A2S query and info checks.
[gamemodules.fofserver.servercfg]
...
run_create_or_skip_disabled "$SERVER_NAME" create fofserver
