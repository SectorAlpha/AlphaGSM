...
SERVER_NAME="${SERVER_NAME:-itts3server}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/ts3server-server"
CONFIG_PATH="$WORK_DIR/alphagsm-ts3server.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-ts3server-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-ts3server-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create ts3server
run_setup_or_skip_steamcmd "$SERVER_NAME" setup -n "$PORT" "$INSTALL_DIR"

run_alphagsm "$SERVER_NAME" start
SERVER_STARTED=1
wait_for_ready "$LOG_PATH" "$START_TIMEOUT_SECONDS" 'ServerQuery created|TeamSpeak 3 Server started|listening|started'
wait_for_info_protocol "$SERVER_NAME" ts3 "$START_TIMEOUT_SECONDS"
