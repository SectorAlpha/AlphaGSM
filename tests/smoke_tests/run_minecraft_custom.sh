#\!/usr/bin/env bash
# ENABLED (BYO): supported with an operator-supplied server jar.
echo "Smoke test for minecraft_custom is ENABLED (BYO) - place a real server jar at <install_dir>/<exe_name> and set exe_name before rerunning setup"
exit 77
...
SERVER_NAME="${SERVER_NAME:-itminecraftc}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/minecraft-custom-server"
CONFIG_PATH="$WORK_DIR/alphagsm-minecraft-custom.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-minecraftc-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-minecraftc-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create minecraft.custom
