...
echo "Smoke test for arma3wastelandserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Arma 3 dedicated server app 233780 before setup/start."
exit 0
...
SERVER_NAME="${SERVER_NAME:-itarma3waste}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/arma3wastelandserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-arma3wastelandserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-arma3waste-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-arma3waste-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create arma3wastelandserver
