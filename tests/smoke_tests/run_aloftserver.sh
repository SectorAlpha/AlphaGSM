...
echo "Smoke test for aloftserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-italoftserve}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/aloftserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-aloftserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-aloftserve-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-aloftserve-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create aloftserver
