...
SERVER_NAME="${SERVER_NAME:-itsaleblazer}"
SERVER_STARTED=0
...
mkdir -p "$WORK_ROOT"
WORK_DIR="$(mktemp -d -p "$WORK_ROOT" saleblazersserver-smoke.XXXXXX)"
HOME_DIR="$WORK_DIR/alphagsm-home"
INSTALL_DIR="$WORK_DIR/saleblazersserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-saleblazersserver.conf"
...
sessiontag = AlphaGSM-saleblazer-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create saleblazersserver
run_setup_or_skip_steamcmd "$SERVER_NAME" setup -n "$PORT" "$INSTALL_DIR"

run_alphagsm "$SERVER_NAME" start
SERVER_STARTED=1
wait_for_ready "$LOG_PATH" "$START_TIMEOUT_SECONDS" 'Connected to Console Window!|Server hosted on port'
