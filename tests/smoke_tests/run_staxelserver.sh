#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 755170.
echo "Smoke test for staxelserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Staxel dedicated server app 755170 before setup/start."
exit 0
...
SERVER_NAME="${SERVER_NAME:-itstaxelserv}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/staxelserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-staxelserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-staxelserv-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-staxelserv-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create staxelserver
