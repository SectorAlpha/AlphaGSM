...
SERVER_NAME="${SERVER_NAME:-itbb2server}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/bb2server-server"
CONFIG_PATH="$WORK_DIR/alphagsm-bb2server.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-bb2server-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-bb2server-IT#
keeplogs = 1

# Keep the smoke server awake for real A2S query and info checks.
[gamemodules.bb2server.servercfg]
...
run_create_or_skip_disabled "$SERVER_NAME" create bb2server
