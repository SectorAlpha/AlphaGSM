...
echo "Smoke test for pixarkserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itpixarkserv}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/pixarkserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-pixarkserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-pixarkserv-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-pixarkserv-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create pixarkserver
