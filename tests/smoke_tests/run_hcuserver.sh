...
SERVER_NAME="${SERVER_NAME:-ithcuserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/hcuserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-hcuserver.conf"
LOG_GLOB="$INSTALL_DIR/Unboxed/Saved/Logs/Unboxed*.log"
...
run_create_or_skip_disabled "$SERVER_NAME" create hcuserver
run_setup_or_skip_steamcmd "$SERVER_NAME" setup -n "$PORT" "$INSTALL_DIR"

run_alphagsm "$SERVER_NAME" start
SERVER_STARTED=1
wait_for_glob_ready "$LOG_GLOB" "$START_TIMEOUT_SECONDS" "listening on port"
