...
echo "Smoke test for inssserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itinssserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/inssserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-inssserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-inssserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-inssserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create inssserver
