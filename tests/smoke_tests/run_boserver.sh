#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 416881.
echo "Smoke test for boserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Blackwake: Overgrowth dedicated server app 416881 before setup/start."
exit 0
...
SERVER_NAME="${SERVER_NAME:-itboserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/boserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-boserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-boserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-boserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create boserver
