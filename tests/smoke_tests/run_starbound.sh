...
echo "Smoke test for starbound is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itstarbound}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/starbound-server"
CONFIG_PATH="$WORK_DIR/alphagsm-starbound.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-starbound-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-starbound-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create starbound
