#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 3017590.
echo "Smoke test for brickadiaserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Brickadia dedicated server app 3017590 before setup/start."
exit 77
...
SERVER_NAME="${SERVER_NAME:-itbrickadias}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/brickadiaserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-brickadiaserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-brickadias-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-brickadias-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create brickadiaserver
