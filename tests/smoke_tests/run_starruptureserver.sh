...
echo "Smoke test for starruptureserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itstarruptur}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/starruptureserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-starruptureserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-starruptur-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-starruptur-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create starruptureserver
