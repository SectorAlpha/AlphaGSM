...
SERVER_NAME="${SERVER_NAME:-ithl2dmserve}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/hl2dmserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-hl2dmserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-hl2dmserve-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-hl2dmserve-IT#
keeplogs = 1

# Keep the smoke server awake for real A2S query and info checks.
[gamemodules.hl2dmserver.servercfg]
...
run_create_or_skip_disabled "$SERVER_NAME" create hl2dmserver
