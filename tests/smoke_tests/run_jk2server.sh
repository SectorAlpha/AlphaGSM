#\!/usr/bin/env bash
# ENABLED (BYO): set url to a working Jedi Outcast dedicated-server archive or
# stage jk2mvded.x86_64 in <install_dir> before setup/start.
echo "Smoke test for jk2server requires ENABLED (BYO) input: set url to a working Jedi Outcast dedicated-server archive or stage jk2mvded.x86_64 in <install_dir> before setup/start."
exit 0
...
SERVER_NAME="${SERVER_NAME:-itjk2server}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/jk2server-server"
CONFIG_PATH="$WORK_DIR/alphagsm-jk2server.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-jk2server-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-jk2server-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create jk2server
