#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 2223650.
echo "Smoke test for motortownserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Motor Town dedicated server app 2223650 before setup/start."
exit 77
...
SERVER_NAME="${SERVER_NAME:-itmotortowns}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/motortownserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-motortownserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-motortowns-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-motortowns-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create motortownserver
