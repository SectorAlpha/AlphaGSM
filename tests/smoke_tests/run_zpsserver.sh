...
echo "Smoke test for zpsserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itzpsserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/zpsserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-zpsserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-zpsserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-zpsserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create zpsserver
