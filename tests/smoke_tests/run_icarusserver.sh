...
echo "Smoke test for icarusserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-iticarusserv}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/icarusserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-icarusserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-icarusserv-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-icarusserv-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create icarusserver
