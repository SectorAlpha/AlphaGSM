#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 696370.
echo "Smoke test for brokeprotocolserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to BROKE PROTOCOL app 696370 before setup/start."
exit 77
...
SERVER_NAME="${SERVER_NAME:-itbrokeproto}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/brokeprotocolserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-brokeprotocolserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-brokeproto-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-brokeproto-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create brokeprotocolserver
