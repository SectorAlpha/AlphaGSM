...
SERVER_NAME="${SERVER_NAME:-itcssserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/cssserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-cssserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-cssserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-cssserver-IT#
keeplogs = 1

# Keep the smoke server awake for real A2S query and info checks.
[gamemodules.cssserver.servercfg]
...
run_create_or_skip_disabled "$SERVER_NAME" create cssserver
