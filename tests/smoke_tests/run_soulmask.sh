...
echo "Smoke test for soulmask is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itsoulmask}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/soulmask-server"
CONFIG_PATH="$WORK_DIR/alphagsm-soulmask.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-soulmask-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-soulmask-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create soulmask
