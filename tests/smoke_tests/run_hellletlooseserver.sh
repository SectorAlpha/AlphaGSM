#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 822500.
echo "Smoke test for hellletlooseserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Hell Let Loose dedicated server app 822500 before setup/start."
exit 0
...
SERVER_NAME="${SERVER_NAME:-ithellletloo}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/hellletlooseserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-hellletlooseserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-hellletloo-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-hellletloo-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create hellletlooseserver
