...
SERVER_NAME="${SERVER_NAME:-itsubsistenc}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/subsistenceserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-subsistenceserver.conf"
LOG_GLOB="$INSTALL_DIR"'/*/Logs/Launch.log'
...
sessiontag = AlphaGSM-subsistenc-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create subsistenceserver
run_setup_or_skip_steamcmd "$SERVER_NAME" setup -n "$PORT" "$INSTALL_DIR"

run_alphagsm "$SERVER_NAME" start
SERVER_STARTED=1
wait_for_glob_ready "$LOG_GLOB" "$START_TIMEOUT_SECONDS" 'Engine is initialized|listening on port|Listening for client'
