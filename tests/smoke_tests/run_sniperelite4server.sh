...
echo "Smoke test for sniperelite4server is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itsniperelit}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/sniperelite4server-server"
CONFIG_PATH="$WORK_DIR/alphagsm-sniperelite4server.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-sniperelit-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-sniperelit-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create sniperelite4server
