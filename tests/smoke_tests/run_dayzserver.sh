#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 223350.
echo "Smoke test for dayzserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to DayZ dedicated server app 223350 before setup/start."
exit 0
...
SERVER_NAME="${SERVER_NAME:-itdayzserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/dayzserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-dayzserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-dayzserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-dayzserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create dayzserver
