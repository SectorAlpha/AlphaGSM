...
echo "Smoke test for primalcarnageextinctionserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itprimalcarn}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/primalcarnageextinctionserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-primalcarnageextinctionserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-primalcarn-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-primalcarn-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create primalcarnageextinctionserver
