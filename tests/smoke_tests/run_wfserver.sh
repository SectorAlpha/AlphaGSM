...
SERVER_NAME="${SERVER_NAME:-itwfserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/wfserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-wfserver.conf"
...
sessiontag = AlphaGSM-wfserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create wfserver
run_setup_or_skip_steamcmd "$SERVER_NAME" setup -n "$PORT" "$INSTALL_DIR"

run_alphagsm "$SERVER_NAME" start
SERVER_STARTED=1
wait_for_info_protocol "$SERVER_NAME" "quake" "$START_TIMEOUT_SECONDS" "$PORT"
