...
echo "Smoke test for q2server is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itq2server}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/q2server-server"
CONFIG_PATH="$WORK_DIR/alphagsm-q2server.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-q2server-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-q2server-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create q2server
