...
echo "Smoke test for notdserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itnotdserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/notdserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-notdserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-notdserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-notdserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create notdserver
