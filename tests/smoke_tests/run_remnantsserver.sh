...
echo "Smoke test for remnantsserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itremnantsse}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/remnantsserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-remnantsserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-remnantsse-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-remnantsse-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create remnantsserver
