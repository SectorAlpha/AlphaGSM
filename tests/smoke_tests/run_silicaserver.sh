...
SERVER_NAME="${SERVER_NAME:-itsilicaserv}"
SERVER_STARTED=0
...
cleanup() {
  ...
  if [[ "$rc" -ne 0 && -n "${INSTALL_DIR:-}" ]]; then
    capture_application_logs "$INSTALL_DIR"/.alphagsm-home/.config/unity3d/*/*/*.log "$INSTALL_DIR"/.alphagsm-home/Silica/*.log
...
INSTALL_DIR="$WORK_DIR/silicaserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-silicaserver.conf"
...
run_alphagsm "$SERVER_NAME" set servername "AlphaGSM Silica Smoke"
