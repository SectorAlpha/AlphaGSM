#!/usr/bin/env bash
# Smoke test: Minecraft Vanilla lifecycle via the GNU screen backend.
# Mirrors run_minecraft_vanilla.sh but explicitly selects backend = screen.
set -Eeuo pipefail
set -x
...
SERVER_NAME="${SERVER_NAME:-bk-screen}"
...
JAVA_WRAPPER="$WORK_DIR/java-wrapper.sh"
...
run_alphagsm "$SERVER_NAME" message "hello from screen backend"
