...
echo "Smoke test for gtafivemserver is ENABLED (BYO) - complete txAdmin/server-data provisioning with server.cfg and a Cfx license key before lifecycle validation"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itgtafivemse}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/gtafivemserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-gtafivemserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-gtafivemse-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-gtafivemse-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create gtafivemserver
