...
echo "Smoke test for ndserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itndserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/ndserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-ndserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-ndserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-ndserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create ndserver
