...
echo "Smoke test for mw3server is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itmw3server}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/mw3server-server"
CONFIG_PATH="$WORK_DIR/alphagsm-mw3server.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-mw3server-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-mw3server-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create mw3server
