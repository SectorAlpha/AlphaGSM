...
SERVER_NAME="${SERVER_NAME:-itblackops3s}"
SERVER_STARTED=0
...
WORK_DIR="$(mktemp -d -p "$WORK_ROOT" blackops3-smoke.XXXXXX)"
HOME_DIR="$WORK_DIR/alphagsm-home"
INSTALL_DIR="$WORK_DIR/blackops3server-server"
CONFIG_PATH="$WORK_DIR/alphagsm-blackops3server.conf"
LOG_PATH="$INSTALL_DIR/UnrankedServer/identities/dedicatedpc/console_mp.log"
...
sessiontag = AlphaGSM-blackops3s-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create blackops3server
run_alphagsm "$SERVER_NAME" set image "$DOCKER_IMAGE"
run_setup_or_skip_steamcmd "$SERVER_NAME" setup -n "$PORT" "$INSTALL_DIR"

run_alphagsm "$SERVER_NAME" start
SERVER_STARTED=1
wait_for_ready "$LOG_PATH" "$START_TIMEOUT_SECONDS" "CreateDedicatedModsLobby: ready!"
