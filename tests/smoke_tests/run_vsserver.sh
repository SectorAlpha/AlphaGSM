...
echo "Smoke test for vsserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itvsserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/vsserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-vsserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-vsserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-vsserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create vsserver
