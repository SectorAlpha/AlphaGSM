#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 380840.
echo "Smoke test for twserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to server app 380840 before setup/start."
exit 77
...
SERVER_NAME="${SERVER_NAME:-ittwserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/twserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-twserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-twserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-twserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create twserver
