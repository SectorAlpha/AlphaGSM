...
echo "Smoke test for askaserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itaskaserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/askaserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-askaserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-askaserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-askaserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create askaserver
