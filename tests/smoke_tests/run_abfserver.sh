...
echo "Smoke test for abfserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itabfserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/abfserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-abfserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-abfserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-abfserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create abfserver
