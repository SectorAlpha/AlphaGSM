...
echo "Smoke test for qlserver is disabled - anonymous SteamCMD setup is not enough; this lane needs an owned/authenticated Quake Live entitlement plus any required server auth/config"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itqlserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/qlserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-qlserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-qlserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-qlserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create qlserver
