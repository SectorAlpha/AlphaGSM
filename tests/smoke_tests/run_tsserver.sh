...
echo "Smoke test for tsserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-ittsserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/tsserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-tsserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-tsserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-tsserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create tsserver
