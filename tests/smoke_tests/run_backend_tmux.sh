#!/usr/bin/env bash
# Smoke test: Minecraft Vanilla lifecycle via the tmux backend.
set -Eeuo pipefail
set -x
...
SERVER_NAME="${SERVER_NAME:-bk-tmux}"
...
run_alphagsm "$SERVER_NAME" message "hello from tmux backend"
