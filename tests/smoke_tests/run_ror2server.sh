#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 1180760.
echo "Smoke test for ror2server is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Risk of Rain 2 dedicated server app 1180760 before setup/start."
exit 0
...
SERVER_NAME="${SERVER_NAME:-itror2server}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/ror2server-server"
CONFIG_PATH="$WORK_DIR/alphagsm-ror2server.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-ror2server-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-ror2server-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create ror2server
