...
echo "Smoke test for pathoftitansserver is ENABLED (BYO) - set auth_token to an Alderon host account token or provide a staged archive override before setup/start"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itpathoftita}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/pathoftitansserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-pathoftitansserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-pathoftita-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-pathoftita-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create pathoftitansserver
