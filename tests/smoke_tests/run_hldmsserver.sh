...
SERVER_NAME="${SERVER_NAME:-ithldmsserve}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/hldmsserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-hldmsserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-hldmsserve-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-hldmsserve-IT#
keeplogs = 1

# Keep the smoke server awake for real A2S query and info checks.
[gamemodules.hldmsserver.servercfg]
...
run_create_or_skip_disabled "$SERVER_NAME" create hldmsserver
