...
echo "Smoke test for tiserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-ittiserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/tiserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-tiserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-tiserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-tiserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create tiserver
