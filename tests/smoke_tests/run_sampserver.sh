...
echo "Smoke test for sampserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itsampserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/sampserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-sampserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-sampserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-sampserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create sampserver
