...
SERVER_NAME="${SERVER_NAME:-itonsetserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/onset-server"
CONFIG_PATH="$WORK_DIR/alphagsm-onset.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-onset-IT#$SERVER_NAME.log"
...
run_create_or_skip_disabled "$SERVER_NAME" create onsetserver
run_setup_or_skip_steamcmd "$SERVER_NAME" setup -n "$PORT" "$INSTALL_DIR"

run_alphagsm "$SERVER_NAME" start
SERVER_STARTED=1
wait_for_ready "$LOG_PATH" "$START_TIMEOUT_SECONDS" 'Server loaded\. Entering simulation'
