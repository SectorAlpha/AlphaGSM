...
echo "Smoke test for chivalryserver is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itchivalryse}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/chivalryserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-chivalryserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-chivalryse-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-chivalryse-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create chivalryserver
