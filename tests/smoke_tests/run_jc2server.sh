...
echo "Smoke test for jc2server is disabled - see docs/TEST_STATUS.md for status"
exit 0
...
SERVER_NAME="${SERVER_NAME:-itjc2server}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/jc2server-server"
CONFIG_PATH="$WORK_DIR/alphagsm-jc2server.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-jc2server-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-jc2server-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create jc2server
