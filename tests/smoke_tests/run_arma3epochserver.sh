...
echo "Smoke test for arma3epochserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Arma 3 dedicated server app 233780 before setup/start."
exit 77
...
SERVER_NAME="${SERVER_NAME:-itarma3epoch}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/arma3epochserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-arma3epochserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-arma3epoch-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-arma3epoch-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create arma3epochserver
