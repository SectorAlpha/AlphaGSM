#\!/usr/bin/env bash
# ENABLED (AUTH): supported with authenticated Steam/SteamCMD entitlement for app 223250.
echo "Smoke test for roserver is ENABLED (AUTH) - authenticate Steam or SteamCMD with an account entitled to Red Orchestra dedicated server app 223250 before setup/start."
exit 0
...
SERVER_NAME="${SERVER_NAME:-itroserver}"
SERVER_STARTED=0
...
INSTALL_DIR="$WORK_DIR/roserver-server"
CONFIG_PATH="$WORK_DIR/alphagsm-roserver.conf"
LOG_PATH="$HOME_DIR/logs/AlphaGSM-roserver-IT#$SERVER_NAME.log"
...
sessiontag = AlphaGSM-roserver-IT#
keeplogs = 1
EOF
...
run_create_or_skip_disabled "$SERVER_NAME" create roserver
