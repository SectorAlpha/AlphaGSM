"""Minecraft Vanilla lifecycle test using the subprocess backend."""

import pytest

pytestmark = pytest.mark.backend_integration

SERVER_NAME = "bk-subproc"
START_TIMEOUT = 180
STOP_TIMEOUT = 90


def test_minecraft_vanilla_subprocess_lifecycle(tmp_path, lifecycle):
    """Full create → setup → start → verify → stop cycle with subprocess backend."""
    ...
    print(f"Minecraft {release_id}, port {port}, backend=subprocess")
