"""Minecraft Vanilla lifecycle test using the GNU screen backend."""

import pytest

pytestmark = pytest.mark.backend_integration

SERVER_NAME = "bk-screen"
START_TIMEOUT = 180
STOP_TIMEOUT = 90


def test_minecraft_vanilla_screen_lifecycle(tmp_path, lifecycle):
    """Full create → setup → start → verify → stop cycle with screen."""
    lifecycle.require_backend_opt_in()
    lifecycle.require_command("screen")
    ...
    lifecycle.write_config(config_path, home_dir, "BkScreen#", backend="screen")
    ...
    print(f"Minecraft {release_id}, port {port}, backend=screen")
    ...
    lifecycle.run_and_assert_ok(env, SERVER_NAME, "message", "hello from screen backend")
